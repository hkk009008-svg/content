...
def make_project(name: str) -> dict:
    return {
        ...
        "global_settings": {
            ...
            # and KLING_NATIVE.storyboard_mode; ADR-080 removed the inert rest)
            # is intentionally NOT scaffolded here: it's operator-opt-in,
            # merged into global_settings only on UI settings-save, seeded from
            # the api_engine_defaults catalog in web_server.py (the source of
            # truth for shape/defaults). Both readers safe-default the absent
            # key (domain/video_engine_policy.py _is_project_disabled,
