...
by that invocation. Do not configure router port forwarding.
...
must equal the random gateway token entered on Windows. A mode-0600
`PERFORMANCE_COMFYUI_API_KEY_FILE` remains supported for a performance-only
configuration; shared FLUX.2 use also requires `COMFYUI_API_KEY` to resolve to
the same value. The authenticated capability contract below keeps each role's
...
and register the at-logon supervisor. Replace the Mac address if DHCP assigned a
different one:
...
& "$InstallRoot\package\Register-WorkerTask.ps1" `
  ...
  -MacIPAddress 192.0.2.2
```

Registration reruns the full execution proof before changing the firewall or
scheduled task. If it reports a conflicting TCP/22 rule, inspect it with
...
Mac IPv4 address, and it will not weaken or disable unrelated access policy. The task runs as the
current user at limited privilege, ignores
duplicate starts, and keeps a single supervisor alive. The readiness sentinel is
removed whenever either child process exits.
