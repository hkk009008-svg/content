...
def test_codex_hook_scripts_are_native_and_bridge_codex_seat():
    ...
    session_smoke = (hooks_dir / "session-smoke.sh").read_text(encoding="utf-8")
    assert "scripts/threeway_readiness.py" in session_smoke
    assert "--hook-summary" in session_smoke
    assert "threeway ready-not-live" in session_smoke

    assert "scripts/threeway_readiness.py" in update_state
    assert "--state-line" in update_state
...
def test_codex_session_smoke_does_not_fallback_to_system_python(tmp_path):
    ...
    assert "threeway ready-not-live" in result.stdout
