...
# TestValidatorSiblingPin lived here. It was a strict xfail standing in for
# identity/validator._resolve_embed_model's dual-channel fix while that fix sat
# unmerged on claude/clever-jepsen-d8074d (fd423ae5). That branch has now landed
# (ADR-078), the pin XPASSed strictly exactly as designed, and its own reason
# line said to remove it on arrival. Coverage was NOT dropped — the successor it
# named took over and asserts the identical property:
#   tests/unit/test_identity_embed_model.py::TestEmbedModelSelection
#       ::test_structural_warning_survives_tf_filter_stomp   (:89)
#       -> assert "STRUCTURAL" in err and "Buffalo_L" in err (:111)
# Deleted rather than un-xfailed, per "prefer deleting mechanism to adding it":
# two copies of one assertion is not two guards.
