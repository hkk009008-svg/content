        ...
        <>
          <div className="space-y-2 mb-3">
            {project.locations.map(l => (
              <div key={l.id} className="bg-app border border-line rounded-lg p-3 group">
                {editingId === l.id ? (
                  <div className="space-y-2">
                    ...
                    <input type="text" value={form.lighting} onChange={e => setForm({ ...form, lighting: e.target.value })}
                      ...
                      className="w-full bg-panel border border-line rounded px-3 py-1.5 text-xs text-tx placeholder:text-mut focus:outline-none focus:border-acc" />
                    <div className="grid grid-cols-2 gap-2">
                      <select value={form.time_of_day} onChange={e => setForm({ ...form, time_of_day: e.target.value })}
                        className="bg-panel border border-line rounded px-2 py-1.5 text-xs text-tx focus:outline-none">
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                          {l.reference_images.map((img: string, i: number) => (
                            <img key={i}
                              ...
                              alt={`Ref ${i + 1}`}
                              className="w-16 h-10 object-cover rounded border border-line" />
                          ))}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => { setEditingId(null); setEditFiles(null) }} className="px-3 py-1.5 text-mut text-xs hover:text-tx">Cancel</button>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-start gap-3">
                    ...
                    {l.reference_images?.length > 0 ? (
                      <img
                        ...
                        alt={l.name}
                        className="w-16 h-10 object-cover rounded border border-line shrink-0"
                      />
                    ) : (
                      <div className="w-16 h-10 rounded border border-dashed border-line bg-panel flex items-center justify-center shrink-0">
                        <span className="text-mut text-eyebrow">No ref</span>
                      </div>
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-tx text-sm">{l.name}</div>
                      <div className="text-mut text-xs mt-0.5 line-clamp-2">{l.description}</div>
                      <div className="flex gap-1.5 mt-1.5 flex-wrap">
                        <span className="text-eyebrow bg-panel px-1.5 py-0.5 rounded text-mut">{l.time_of_day}</span>
                        {l.weather !== 'clear' && (
                        ...
                        <span className="text-eyebrow bg-panel px-1.5 py-0.5 rounded text-mut">seed: {l.seed}</span>
                      </div>
                    </div>
                    <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 shrink-0">
                      <button onClick={() => startEdit(l)} className="text-acc hover:text-acc text-xs">Edit</button>
                      <button onClick={() => handleDelete(l.id)} className="text-mut hover:text-fail text-xs">Remove</button>
                    </div>
                  </div>
                )}
              </div>
          ...
          {adding ? (
            <div className="bg-app border border-acc/30 rounded-lg p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-xs text-mut block mb-1">Time of Day</label>
                  <select value={form.time_of_day} onChange={e => setForm({ ...form, time_of_day: e.target.value })}
                ...
                <div>
                  <label className="text-xs text-mut block mb-1">Weather</label>
                  <select value={form.weather} onChange={e => setForm({ ...form, weather: e.target.value })}
                    className="w-full bg-panel border border-line rounded px-3 py-2 text-sm text-tx focus:outline-none">
                    {weatherOptions.map(w => <option key={w} value={w}>{w}</option>)}
                  </select>
              ...
              <div>
                <label className="text-xs text-mut block mb-1">Reference photos (optional)</label>
                <input type="file" accept="image/*" multiple onChange={e => setFiles(e.target.files)}
                  className="w-full text-xs text-mut file:mr-2 file:py-1 file:px-3 file:rounded file:border-0 file:bg-panel file:text-tx file:text-xs" />
              </div>
              <div className="flex gap-2">
                ...
                <button onClick={() => setAdding(false)} className="px-4 py-2 text-mut text-sm hover:text-tx">Cancel</button>
              </div>
            </div>
          ) : (
