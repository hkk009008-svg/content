...
| 15b LTX transition helpers | EVIDENCE COMPLETE 2026-08-01 — `ltx_native.py:276,453`; `git grep -nE '_fal_transition\|_native_transition' -- '*.py'` returns definitions + their own tests only, no dynamic dispatch. Transitions ARE shipped, via ffmpeg `xfade_concat` (`cinema_pipeline.py:1487`) with `transition_duration` PATCH-validated at `web_server.py:1342` — so these are an abandoned ALTERNATIVE, not a pending feature | Slice 4 + full matrix | — | READY TO DELETE — product call: deleting also drops the slice-4 money-gate duration tests, which are correct but guard unreachable code |
| 15c face-validator helpers | NOT A DELETE CANDIDATE 2026-08-01 — `face_validator_gate.score_candidate` is LIVE: imported by `prep/lora_quality.py:202`, `scripts/_fal_lora_production.py:19`, `scripts/_prod_multiscene.py:17`, `scripts/_synth_test.py:20`. A per-function `git grep -c ... \| wc -l` sweep reported all 8 helpers unreferenced; that was a HARNESS ARTIFACT (`-c` prints one line per file; the pathspec exclusions collapsed it to 0), not a finding. A plain module-name grep found the importers immediately | Slice 7 + full matrix | — | CLOSED — no deletion |
