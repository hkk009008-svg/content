...
| **P5** | Identity QC | Swap ArcFace embedding for **AdaFace** (MIT) inside the existing DeepFace harness — **item 1 (adapter) LANDED 2026-07-11**: `identity/adaface.py` + vendored net, selectable via `IDENTITY_EMBED_MODEL=AdaFace`, ir_101/MS1MV2 ckpt via `scripts/download_adaface_ckpt.py`. **Item 2 (pod paired calibration on the ADR-025 ref sets) still owed — do NOT flip the default before it**; the structural uncalibrated warning keeps firing. | Better on degraded/low-quality faces (95.67 vs 94.25 TAR@FAR=0.01% IJB-B; 72.29% Rank-1 TinyFace) — matches over-cooked-render QC; pitfall: 112×112 **BGR** input (handled + test-pinned in the adapter). buffalo_l is easier but non-commercial license | HIGH |
