...
- **IdentityValidator is a process singleton** via `identity.get_shared_validator()` (the canonical factory introduced in commit cc34870). `phase_c_vision._get_shared_validator()` is kept as a backward-compat alias that delegates. `face_validator_gate.py` and `performance/identity_gate.py` also delegate, so all four access paths return the same instance — ArcFace weights load once, rolling-stats history accumulates signal from every identity check in the pipeline.
