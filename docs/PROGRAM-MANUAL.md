...
- **Storyboard batch (cross-shot consistency):** `api_engines.KLING_NATIVE.storyboard_mode=true` — for scenes of **2–6 unapproved shots that all have approved keyframes**, generates ONE Kling clip with cross-shot `image_references`, then splits it (`cinema/phases/motion_render.py:359`). *Caveat: `pipeline_status.toml` flags `storyboard_mode` as a stubbed toggle in some builds — verify it fires on your version before relying on it.*
