...
| Name | file:line | What it does |
|---|---|---|
| `generate_ai_video` | `phase_c_ffmpeg.py:53` | Central dispatch + cascade. Per-engine branches (KLING_NATIVE `:197`, SORA_NATIVE `:227`, VEO_NATIVE `:265`, LTX `:292`, RUNWAY_GEN4 `:331`, SORA_2 `:386`, VEO/FAL `:437`, KLING_3_0 `:511`, FAL_SVD `:597`, RUNWAY `:658`, SEEDANCE `:691`). Returns path or `None`. |
...
**KEY FUNCTIONS:** `generate_ai_video` (`phase_c_ffmpeg.py:53`); inner `try_next_api` (`phase_c_ffmpeg.py:138`) and `_record_video_cascade` (`phase_c_ffmpeg.py:107`); the dialogue override + `audio_embedded` tagging + mandatory lipsync at `cinema/shots/controller.py:1113-1253`.
...
The cascade is fault-tolerant: `generate_ai_video` (`phase_c_ffmpeg.py:53`) tries the primary, and on failure walks the fallback list (`try_next_api`, `phase_c_ffmpeg.py:138`), skipping already-attempted engines. On total exhaustion it sleeps 30 s and retries up to `cascade_retry_limit` (default 1).
...
| `cascade_metadata` (on take) | `_record_video_cascade` (`phase_c_ffmpeg.py:107`) | audit / `audio_embedded` decision | which engine actually won + attempt list |
...
Video generation is the most fault-tolerant subsystem because vendor APIs fail, rate-limit, and reject inputs constantly. The single entry point is `generate_ai_video` (`phase_c_ffmpeg.py:53`); inside it, the closure `try_next_api` (`phase_c_ffmpeg.py:138`) implements an ordered, fault-tolerant cascade.
