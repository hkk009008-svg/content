...
| `update_scene_shots` | `domain/scene_decomposer.py:887` | Persists shots via `mutate_project`. Called by `cinema_pipeline.py:960` and `web_server.py:1388`. |
...
| `lipsync_overlay` / `lipsync_generation` / `generate_lip_sync_video` | `lip_sync.py:697` / `:468` / `:682` | Overlay cascade (SyncV3→MuseTalk→LatentSync→SyncV2); generation cascade (Hedra→Kling→Omnihuman→Aurora); smart router (`mode="auto"`). SyncNet quality gate via `_sync_gate_settings` (`:427`). |
| `generate_rife_interpolation` / `upscale_video_seedvr2` | `lip_sync.py:806` / `:791` | Cloud RIFE (FPS multiplier); cloud SeedVR2 upscale (1080p/2160p). |
