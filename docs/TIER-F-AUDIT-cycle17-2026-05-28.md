...
|---|---|---|
| **NEW-1: `camera_motion_native` inert toggle (UI lie)** | **CLOSED** (ADR-080) | Was: written only in `_API_ENGINE_DEFAULTS`, surfaced in UI, **read by no Python**. The UI half lapsed with the 2026-07 Resolve redesign (`ApiEnginesSection.tsx` deleted; no control renders it). Re-found 2026-07-31 (slice 9b) as dead *schema*, together with the 5 siblings below. Both halves removed: `_API_ENGINE_DEFAULTS` + `ApiEngineConfig` now carry only `enabled` (+ KLING_NATIVE `storyboard_mode`). |
| **NEW-1b: 5 sibling inert `api_engines` sub-fields** | **CLOSED** (ADR-080) | KLING `face_consistency`, SORA/VEO/KLING_3_0/RUNWAY `duration`, SORA/LTX/SEEDANCE/RUNWAY `resolution`, VEO/KLING_3_0 `generate_audio`. Independently confirmed twice (2026-05-28 mailbox convergence; 2026-07-31 slice 9b) and re-verified at `8dde7576`: only 3 sites repo-wide index a per-engine dict (`web_server.py:477`, `motion_render.py:73`, `video_engine_policy.py:273`) and they read 2 keys total. NOT to be confused with the `face_consistency` *API param*, which is correctly wired (§3 note below) — the dead thing was the **setting**, never read; the param is derived from `shot_type` at `phase_c_ffmpeg.py:571`. |
