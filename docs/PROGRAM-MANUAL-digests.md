...
**`get_project_setting(ctx, key, default)`** — `cinema/context.py:146`
...
**`record_director_review_on_shots`** — `cinema/auto_approve.py:202`
...
4. **`headless=True` does NOT use `NullLifecycle`.** The docstring comment in `cinema/lifecycle.py:79` is clear: `NullLifecycle` was the former CLI's lifecycle (for the now-deleted `main.py`). `headless=True` still uses `ThreadedLifecycle` — it just makes `ReviewController._wait_for_gate` raise `GateNotSatisfiedError` instead of polling forever. Any doc or skill claiming headless uses `NullLifecycle` is wrong.
...
12. **`storyboard_mode` flag is at `global_settings.api_engines.KLING_NATIVE.storyboard_mode`**, a nested path two levels deep. `_get_storyboard_mode()` in `cinema/phases/motion_render.py:46` handles the nesting. Accessing `global_settings.storyboard_mode` directly would silently return `None`.
...
- `scene_decomposer.py:1-9` — shim re-export from `domain/scene_decomposer.py`
- `dialogue_writer.py:1-9` — shim re-export from `domain/dialogue_writer.py`
- `project_manager.py:1-9` — shim re-export from `domain/project_manager.py`
...
**`PerformanceCapturePhase.__init__`** (`performance.py:25-33`) — same three params; `shot_generator` must expose `generate_performance_take(scene_id, shot_id)`.

**`PerformanceCapturePhase.run`** (`performance.py:35-89`) — Three skip conditions per shot: (1) `shot["approved_performance_take_id"]` is truthy; (2) `(shot["performance_engine"] or "").upper() == "SKIP"` (explicitly routed away by the domain router); (3) `not shot["approved_keyframe_take_id"]` (no anchor image yet — motion would also skip). Calls `self._gen.generate_performance_take(scene_id, shot_id)`. Also handles `result.get("skipped")` — a result can succeed but report itself as skipped (SKIP-engine path returns from within the ShotController). Always returns `ok=True`; partial failures do not fail the phase.
...
7. **`motion_render.py` references `base.py:109`** comments like "F2b" and "Tier F NEW-2" — internal planning terminology from migration slices; these are not external API names.
...
| `generate_performance_take` delegate | `cinema_pipeline.py:238-239` |
| `generate_motion_take` delegate | `cinema_pipeline.py:241-242` |
| `_resolve_take_path` delegate | `cinema_pipeline.py:299-300` |
| `cost_tracker` property | `cinema_pipeline.py:139-140` |
...
- `RunState` (dataclass) — `cinema/runstate.py:59`. Fields: `shot_results`, `review_clips`, `scene_clips`, `scene_audio`, `scene_foley`, `foley_audio_paths`, `failed_shots`, `current_stage/scene_id/shot_id`, `headless: bool`, `completed_scene_indices`. Instantiated once per `CinemaPipeline.__init__`, passed by reference to all three controllers (`cinema_pipeline.py:91`). `headless=True` is set by the caller; all gate code reads it.
- `update_progress_pointer(stage, scene_id, shot_id)` — `cinema/runstate.py:140`. Atomically sets all three progress-pointer fields. Called by ShotController during generation; orchestrator sets fields directly for single-field cases.
...
- `ReviewControllerHost` (Protocol) — `cinema/review/controller.py:103`. Declares the three cross-controller methods the host (CinemaPipeline) must provide: `_refresh_project_snapshot`, `_find_take`, `_mutate_shot`, `resume`.
...
- `record_director_review_on_shots(shots, review)` — `cinema/auto_approve.py:202`. **The writer** for `shot["director_review"]`. Called at `cinema_pipeline.py:959` immediately after `ChiefDirector.validate_shot_prompts`. Normalises MODIFIED→APPROVED (cycle-17 decision to auto-clear so headless runs don't dead-end). Stores raw verdict in `chief_director_verdict`. **Critical**: this function's absence was the root cause of the cycle-17 headless plan-review-gate stall.
- `_rules_for_plan(config)` — `cinema/auto_approve.py:170`. Two veto rules: `plan_decision_not_approved` (fires if `shot["director_review"]["decision"] != "APPROVED"`); `plan_has_violations` (fires if `director_review["violations"]` non-empty).
- `_rules_for_image(config)` — `cinema/auto_approve.py:247`. Three rules: `image_composite_below_threshold` (dynamic: uses `image_min_composite_fallback` when any take has fallback engine, else `image_min_composite`); `image_cascade_fallback` (fires if any take has `cascade_metadata.fallback=True`); `image_over_budget` (fires if `shot.spent_usd > multiplier × per_shot_budget`).
- `_rules_for_motion(config)` — `cinema/auto_approve.py:307`. Three rules: `motion_identity_below_threshold`, `motion_score_below_threshold` (uses `motion_fidelity` or `motion_score` from metadata), `motion_cascade_fallback`.
- `_rules_for_final(config)` — `cinema/auto_approve.py:351`. Two rules: `final_lipsync_below_threshold` (dialogue-aware: returns 1.0 for non-dialogue shots, 1.0 for `audio_embedded=True` takes, 0.0 for dialogue with no score and no audio_embedded); `final_upstream_was_auto_approved` (safety net: if `plan_auto_approved`, `image_auto_approved`, or `motion_auto_approved` is True on the shot, require human at final).
- `check_gate(gate, *, shot_state, project, takes, config)` — `cinema/auto_approve.py:582`. Public entry point. Returns `AutoApproveDecision`. All exceptions caught; eval-error sets `deferred=True`. Returns `auto_approved=False` if `config.enabled=False`.
- `pick_best_take_by_composite(takes)` — `cinema/auto_approve.py:477`. Returns take with highest `metadata.composite`; falls back to `takes[-1]` if none have a score. Used by controller's image approval mutation.
- `pick_best_take_for_final(candidates)` — `cinema/auto_approve.py:508`. Prefers non-fallback takes first; then picks by composite score. Used by controller's final approval mutation.
- `is_motion_gate_enabled()` — `cinema/auto_approve.py:568`. Reads `CINEMA_AUTO_APPROVE_MOTION` env var (case-insensitive `1|true|yes`). Off by default.
- `summarize_audit(shot_state)` — `cinema/auto_approve.py:693`. Aggregates `shot["auto_approve_audit"]` into a Session-12 PostRunSummary dict (auto_approved_gates, vetoed_gates, per_rule_fire_counts).

**cinema/screening.py**

- `_screening_stage_enabled(project=None)` — `cinema/screening.py:84`. Feature flag. Priority: (1) `project["global_settings"]["screening_stage_enabled"]` (lenient bool/int/str parsing); (2) `CINEMA_SCREENING_STAGE` env var (falsy: `0|false|no`); (3) default ON. Default ON as of v5.1+ flag-flip.
- `_build_timeline_manifest(project, *, verify_files=False)` — `cinema/screening.py:157`. Walks scenes/shots with `approved_final_take_id`; builds list of `{shot_id, scene_id, start_s, end_s, approved_take_id, take_count}`. Duration: `take.metadata.duration_s` > `scene.duration_seconds` > 5.0. With `verify_files=True` (used by the `/assemble/screen` endpoint): also checks `os.path.exists(take.path)` before including a shot, mirroring `_assemble_final`'s inclusion rule.
- `is_screening_approved(project)` — `cinema/screening.py:275`. Reads `project.get("screening_approved", False)`.
- `mark_screening_approved(project_id)` — `cinema/screening.py:287`. Lazy-imports `mutate_project`; runs `Project.model_validate` inside mutator (P1-3 Variant 1 shape); sets `project["screening_approved"] = True`. Raises `ValueError` if project not found.
- `get_needs_reassembly(project)` — `cinema/screening.py:333`. Returns `project["needs_reassembly"]` as a list; defensive against non-list values.
- `mark_shot_needs_reassembly(project_id, shot_id)` — `cinema/screening.py:351`. Idempotent add to `project["needs_reassembly"]`. Called by `ShotController.regenerate_with_intent` when an iterate fires during SCREENING.
- `clear_needs_reassembly(project_id, only_shots=None)` — `cinema/screening.py:401`. With `only_shots=None`: wipes entire list. With `only_shots` list: only removes those shot_ids, preserving any concurrently added entries (Lane V #8 I3 close).
- `estimate_reassembly_cost(project)` — `cinema/screening.py:498`. Heuristic: `cost_s ≈ shot_count × 0.5 + total_duration_s × 0.09`, floor 5s. Returns `{seconds, shot_count, total_source_duration_s, breakdown}`.
...
2. **`generate(resume=False)`** (`cinema_pipeline.py:844`): if `resume=True`, calls `_restore_from_checkpoint()` which reads `pipeline_state.json` and wholesale-assigns all RunState fields back.
...
7. **MotionRenderPhase** runs; `_rebuild_review_clips(project)` builds in-memory manifest; `_save_checkpoint()`; **REVIEW gate** (`cinema_pipeline.py:1084`): auto-approve evaluates `_rules_for_final` on `postprocess_variants + motion_takes`; if approved, writes `approved_final_take_id` (and `approved_motion_take_id`) via `pick_best_take_for_final` (prefers non-fallback, then highest composite). `final_require_human_if_upstream_auto` veto fires if any earlier gate was auto-approved.
...
3. **PERFORMANCE_REVIEW auto-approve is opt-in via env var**: without `CINEMA_AUTO_APPROVE_MOTION=1`, the motion gate map entry is omitted (`controller.py:280-282`) and the PERFORMANCE_REVIEW gate is always manual. The `_rules_for_motion` function exists and is used when enabled, but the gate is silently not included by default.
...
7. **`_rebuild_review_clips` is in-memory only**: `runstate.review_clips` is rebuilt on each run start (from checkpoint or from scratch). It is NOT persisted in the checkpoint. Web endpoints that read `review_clips` work only while the pipeline is running. After process restart, the checkpoint restores other RunState fields but NOT `review_clips` — the separate call `_rebuild_review_clips(project)` at `cinema_pipeline.py:857` handles this on resume.
...
10. **`summarize_audit` is pre-staged but not yet wired to any endpoint**: it is defined at `cinema/auto_approve.py:693` for "Session 12's PostRunSummary modal" but no web endpoint calls it in the current codebase (unverified as of the files read — no grep result in web_server.py for `summarize_audit`).
...
| `prompt_optimizer_enabled` | `True` | LLM-based shot-prompt rewrite before generation; caches to `shot["optimizer_cache"]` | `shots/controller.py:391` |
...
- `project_manager.py:9` — shim: `from domain.project_manager import *`
...
**`_heuristic_purpose_with_objects(shot_type, has_chars, has_dialogue, has_objects, primary_subject)`** — `prompt_optimizer.py:38`
...
**`_default_style_rules(mood, color_palette, music_mood)`** — `style_director.py:122`
...
**`style_rules_to_prompt_suffix(style_rules)`** — `style_director.py:182`
...
**`_to_str(val)`** — `style_director.py:171`
...
| `domain/scene_decomposer.py` | Imports `LLMEnsemble` at `scene_decomposer.py:11`; calls `competitive_generate()` at ~749 for competitive scene decomposition |
...
| Dual-module shims all 9-LOC re-exports | `scene_decomposer.py:1-9`, `character_manager.py:1-9`, etc. |
...
**`get_language_defaults`** — `domain/language_defaults.py:130`
...
**`merge_language_defaults_into_settings`** — `domain/language_defaults.py:151`
...
| `web_server.py` | Direct call | `api_decompose_scene` at `web_server.py:1350` calls `decompose_scene`; `api_generate_dialogue` at `web_server.py:1313` calls `generate_dialogue`; `api_apply_language_defaults` at `web_server.py:384` calls `merge_language_defaults_into_settings` |
...
| `domain/project_manager.py` (`make_shot`, `mutate_project`) | Direct call | `make_shot` at `domain/scene_decomposer.py:593,807`; `mutate_project` via `update_scene_shots` at `domain/scene_decomposer.py:912` |
...
**1. `api_decompose_scene` never uses competitive path.** The HTTP endpoint at `web_server.py:1381` calls `decompose_scene()` directly, not `competitive_decompose_scene()`. Only the automated pipeline (`cinema_pipeline.py:921-932`) respects `competitive_generation`. On-demand decompose via UI is always single-model.
...
**4. Shim doc comment mentions `main.py`, `cleanup.py` as callers.** `main.py` is deleted (confirmed by MEMORY.md). The shim's docstring (`scene_decomposer.py:5`) is stale on this point.

**5. `PIPELINE_CONTEXT` includes stale lipsync routing.** `config/prompts/pipeline_context.md:19` says "Dialogue close-up → VEO_NATIVE / Native lip-sync", but `PIPELINE_CONTEXT` section 2 lines 42-44 describe Omnihuman v1.5 as PRIMARY and Veo as OPT-IN only (RAI filter concern). The API routing table (section 1) conflicts with section 2's cascade. Decomposer LLM sees both and must reconcile; the hard-coded `PURPOSE_API_RANKING["dialogue_close_up"]` in code (`domain/scene_decomposer.py:121`) leads with `HEDRA_C3`, not `VEO_NATIVE`.

**6. Shot JSON schema description in `competitive_decompose_scene` differs from `decompose_scene`.** At `domain/scene_decomposer.py:714`, the `prompt` field description says "including ALL character physical descriptions and FULL location description" — contradicting HC1 (identity firewall). The `decompose_scene` version at `domain/scene_decomposer.py:518` correctly says "leaving facial identity to reference locking". This is a latent prompt contamination risk in the competitive path's schema.
...
**8. `update_scene_shots` uses `MutationResult(False, save=False)` only when scene not found.** Return value is `True` on success but ignored by callers. The `timeout=10` default is overridden to `HTTP_PROJECT_TIMEOUT` in the web path (`web_server.py:1388`) but not in the pipeline path (`cinema_pipeline.py:960`).
...
- `scene_decomposer.py:1-9` — shim definition, lists legacy callers (including stale `main.py`)
...
Writes `{"engine": winning_engine, "attempts": [...]}` into `_cascade_out["cascade_metadata"]`. Called immediately before every successful engine `return`. Not called on fallback. Enables the caller (`shots/controller.py:1205`) to persist provenance to the take record.
...
- Singleton accessor `get_ltx_client()` at `ltx_native.py:348`.
...
|---|---|---|
| `cinema/shots/controller.py` | Direct call | Primary caller at `controller.py:1186`; builds `PipelineContext`, resolves `target_api` + `video_fallbacks`, passes `driving_video_path` + `has_dialogue`; reads `_cascade_out` post-call |
...
| `lip_sync.py` | Post-call (F1b) | Called at `controller.py:1233` when `has_dialogue=True` and `not audio_embedded` — mandatory lipsync pass |
...
1. **Shot classification**: `classify_shot_type(shot)` → `shot_type` ∈ {portrait, medium, wide, action, landscape} (`controller.py:1083`)
...
3. **`generate_ai_video` called** (`controller.py:1186`) with resolved `target_api`, `video_fallbacks`, `driving_video_path`, `has_dialogue`, `ctx`, `_cascade_out={}`
...
| `generate_video` 180-s timeout kwarg pop | `kling_native.py:288` |
...
#### `RunPodComfyUI` — `phase_c_assembly.py:32`
...
#### `ImageGenResult` — `phase_c_assembly.py:15`
...
#### `_fal_flux_fallback` — `phase_c_assembly.py:415`
...
#### `_parse_structured_prompt` — `phase_c_assembly.py:397`
...
#### `generate_ai_broll_max` — `quality_max.py:574`
...
#### `_probe_node_availability` — `quality_max.py:250`
...
#### `_prune_unavailable` — `quality_max.py:340`
...
#### `_inject_identity` — `quality_max.py:397`
...
#### `_inject_conditioning` — `quality_max.py:419`
...
#### `_inject_sampling` — `quality_max.py:453`
...
#### `_inject_latent_source` — `quality_max.py:474`
...
#### `_inject_post_passes` — `quality_max.py:497`
...
#### `_swap_to_hidream` — `quality_max.py:197`
...
#### `_validate_overlay_value` — `quality_max.py:141`
...
#### `should_halt` — `face_validator_gate.py:225`
...
#### `score_candidate` — `face_validator_gate.py:168`
...
#### `needs_regenerate` — `face_validator_gate.py:289`
...
#### `generate_keyframe_take` — `cinema/shots/controller.py:314`
...
|---|---|---|---|
| `identity_strictness` | 0.60 | Override threshold for keyframe image validation in `controller.py:504` and N=8 scoring in `quality_max.py:767` | Set to 0.70–0.75 for portrait shots |
...
**`coherence_threshold`** is read in `controller.py:1667` with a fallback of 0.6, but it is NOT scaffolded in `domain/project_manager.py`'s default `global_settings` dict. Must be set manually via `PUT /api/projects/<pid>`.

**`CharacterContinuityTracker` holds dict references** (not model_dump copies) to `project["characters"][i]`. This is intentional (preserves mutation visibility) but means that if upstream code replaces the project dict entirely (as `_refresh_project_snapshot` does in `cinema_pipeline.py:431`), the tracker's `self.characters` becomes stale. The pipeline code re-instantiates the tracker on snapshot reload.
...
**`_get_shared_validator`** (`phase_c_vision.py:15`)
...
**`get_middle_frame`** (`phase_c_vision.py:29`) / **`extract_frame_at`** (`phase_c_vision.py:40`)
OpenCV frame extractors. `extract_frame_at` takes a `position_ratio` (0.0–1.0).

**`face_swap_video_frames`** (`phase_c_vision.py:52`)
...
**`PrerequisiteResult`** (`lip_sync.py:44`)
Dataclass: `passed: bool, mode: str, warnings: List[str], blockers: List[str]`.

**`check_overlay_prerequisites`** (`lip_sync.py:52`)
...
**`check_generation_prerequisites`** (`lip_sync.py:117`)
...
**`lipsync_overlay`** (`lip_sync.py:175`)
...
**`validate_lipsync_quality`** (`lip_sync.py:371`)
...
**`_sync_gate_settings`** (`lip_sync.py:427`)
...
**`lipsync_generation`** (`lip_sync.py:468`)
...
**`upscale_video_seedvr2`** (`lip_sync.py:791`)
...
**`extract_last_frame`** (`lip_sync.py:847`)
...
**`DEFAULT_TRAIN_CONFIG`** (`prep/lora_training.py:99`)
...
**`LoraStatus`** (`prep/lora_training.py:116`)
...
**`prepare_character_lora_dataset`** (`prep/lora_training.py:153`)
...
**`train_character_lora`** (`prep/lora_training.py:376`)
...
**`generate_cartesia`** (`audio/dialogue.py:45`)
...
**`_resolve_tts_provider`** (`audio/dialogue.py:122`)
...
**`_try_dialogue_mode`** (`audio/dialogue.py:177`)
...
**`generate_dialogue_voiceover`** (`audio/dialogue.py:286`)
...
**`_maybe_save_alignment`** (`audio/dialogue.py:252`)
...
**`master_music`** (`audio/music.py:379`)
...
**`_build_foley_prompt`** (`audio/foley.py:29`)
...
**`generate_stability_foley`** (`audio/foley.py:109`)
...
**`WordTiming`** / **`AlignmentResult`** (`audio/alignment.py:38`)
...
| `domain/scene_decomposer.py` (API_REGISTRY) | **Direct reference** — `controller.py:1081` imports `API_REGISTRY` to check `native_audio` flag on winning engine; VEO_NATIVE is the only live engine with `native_audio: True` (`scene_decomposer.py:43`) |
| `phase_c_assembly.py` (image gen) | **Direct import** — `controller.py:90` imports `generate_ai_broll`; used for keyframe generation in Phase B |
...
11. **BGM mastering uses `generate_fal_bgm` directly** in `_ensure_bgm` (`cinema_pipeline.py:531`), bypassing `generate_bgm`'s Suno→FAL router. To use Suno V5 for BGM, the pipeline would need to call `generate_bgm` with `prefer_provider="AUTO"` instead.
...
**`_detect_provider(model: str) -> str`** — `cost_tracker.py:97`
...
Walks `temp/`, `characters/`, `locations/`, `exports/` subdirs. Returns dict with MB values per subdir + total. Used by `api_disk_usage` at `web_server.py:2496`.
...
**`Settings` (frozen dataclass)** — `config/settings.py:48`
...
**`get_settings() -> Settings`** — `config/settings.py:136`
...
| `cleanup_project` aggressive mode | `cleanup.py:83-84` |
| `get_project_disk_usage` subdirs | `cleanup.py:143-145` |
| `_JsonFormatter` extra-field promotion | `cinema/logging_config.py:84-92` |
...
| `build_pipeline_core` budget_usd extraction | `cinema/core.py:99-113` |
...
| `api_cost_live` re-uses cached core tracker | `web_server.py:2509-2514` |
| `api_cleanup` endpoint params | `web_server.py:2480-2492` |
| `api_cleanup_all` endpoint | `web_server.py:2526-2533` |
...
| root `project_manager.py` is a shim | `project_manager.py:1-9` |
