...
- [ ] **Step 1: Write the failing test** — overlay dialogue keeps fallbacks non-None + VEO primary; native dialogue keeps `None`. *(Match the existing routing-assertion style in `test_dialogue_routing.py`.)*
- [ ] **Step 2: Run — expect FAIL.**
...
2. **Duration sizing:** for an overlay-mode dialogue shot, resolve the per-shot TTS BEFORE `generate_ai_video`, measure its duration, and set the Veo `duration` string (`veo_native.generate_video(duration:str="8s")`) to the nearest supported value ≥ speech length, clamped to the supported set `{"4s","6s","8s"}` (cap at `8s`; over-length tails truncate — flagged out-of-scope). When no per-shot line exists, keep the shot's configured duration. *(Implementer: confirm the duration param threads from the `generate_ai_video` call at `:1186` into `veo_native`; grep `duration` in `phase_c_ffmpeg.py`/`veo_native.py`. Use ffprobe or the existing audio-duration util — grep for one before adding a dep.)*
