...
***v4.1 FOLLOWUP SHIPPED 2026-05-25** (cycle 6 close, single chore(protocol) commit, mirrors v2.1 shape). Codifies operator's two §"v4.1 candidate clarifications" from S13 Lane V verification-report: CC-1 (Lane V coalescing rule — operator MAY coalesce ≤5 tightly-coupled commits) + CC-2 (spec-reviewer prompt-construction discipline requiring grep / Read verification of any existence claim before report inclusion). v4.1 SHA placeholder `_Protocol Bundle v4.1 ship_` in PROTOCOL-RULES-LOG.md; operator fills at next session-close.*
