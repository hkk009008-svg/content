...
3. **Cost-tracker silence** — `generate_cartesia` does NOT call `cost_tracker.record_api_call("CARTESIA_SONIC_2")` directly; cost-tracker entry `CARTESIA_SONIC_2: 0.008` was added at v0.9.7 in `cost_tracker.py:62` but is currently invocation-only (manual `record_api_call` from caller). Pipeline integration with auto-tracking would be a future enhancement. Verification: post-Tier-C check that observed Korean dialogue spend matches expected `(N_korean_lines × $0.008)` if a manual record_api_call is added at the dispatcher.
...
- Cost-tracker silence → add `record_api_call("CARTESIA_SONIC_2", shot_id=...)` at the Cartesia success branch in `generate_dialogue_voiceover` dispatcher (small follow-up; not in v0.9.7 scope).
