...
- **v1.0 SHIP — 2026-05-27 cycle-15 entry end-of-session, post-pod-restart-completion + A5/A9 GREEN.** RunPod pod `525nb9d5cc0p3y` fully deployed + serving at `https://525nb9d5cc0p3y-8188.proxy.runpod.net` with FLUX1-dev-fp8 + PuLID-FLUX v0.9.1 + EVA-CLIP + InsightFace antelopev2 + IPAdapter. Pre-flight A5 200/A9 4-of-4 PuLID-Flux nodes + 7-of-7 FLUX core nodes + 3-of-3 IPAdapter nodes + checkpoints/VAE inventory all GREEN. Pod deploy required 6 manual hardening steps NOT in `setup_runpod.sh` (tracked for cycle-16+ script update): `--ignore-installed blinker` for distutils conflict; torch pin `2.4.1+cu118` (unpinned in script; latest crashes with comfy_kitchen); `pip install -r ComfyUI/requirements.txt` unconditionally (script gates on `if [ ! -d ComfyUI ]` which skips on restart when /workspace persists but container deps are wiped); pin torch AFTER ComfyUI requirements.txt to avoid clobber; swap PuLID variant from `cubiq/PuLID_ComfyUI` (SDXL — wrong) to `balazik/ComfyUI-PuLID-Flux` (FLUX); download `pulid_flux_v0.9.1.safetensors` + EVA-CLIP + antelopev2 instead of SDXL pulid weight. Brief substantively v1.0; ALL gates closed pending only user-principal execution authorization signal. **Brief is now executable.** **DRAFT v0.9.8** — operator follow-up fold per user-principal direction "fix" cycle-15 entry: I-2 closure from cycle-15 code-quality review on v0.9.7 Cartesia stack — dispatcher Cartesia success branch (`audio/dialogue.py:357-376`) now wires `cost_tracker.CostTracker().record_api_call("CARTESIA_SONIC_2", operation="dialogue_tts")` per-line, replacing dormant cost-tracker entry with live auto-tracking. Best-effort wrapper mirrors `cinema/shots/controller.py:551-554` pattern (CostTracker failure → log + continue; TTS success not regressed). 2 new unit tests in `TestDispatcherIntegration` (verifies record_api_call invocation on success + verifies dispatcher continues when CostTracker constructor raises). Brief PR-DIALOGUE cell failure-mode #3 rewritten from "Cost-tracker silence" (pending state) to "Cost-tracker integration" (CLOSED state) with explicit asymmetry note: ElevenLabs path remains pre-existing untracked (no `API_COST_USD` entry, no callers across codebase) — symmetric ElevenLabs tracking deferred as future small-scope improvement. Cycle-15 v0.9.7 code-quality reviewer's I-1 (redundant getattr) + I-3 (voice_id slice brittleness) + spec-reviewer F-1 (try/except scope) accepted as advisory per Rule #15 NO ACTION recommendations. **DRAFT v0.9.7** — operator follow-up fold per user-principal direction (Cartesia Sonic 2 TTS re-introduction, cycle-15 entry): `generate_cartesia` re-added to `audio/dialogue.py` (NOT `audio/voiceover.py` — voiceover is caption-only per Bundle-D 4.3 split) with explicit caller integration, addressing the "zero live callers" reason cited in the original Bundle-D 4.3 removal at `48f2a24` on 2026-05-24. Three-commit slice (Phase 1 → Phase 3) ships the function + language router + per-line dispatcher: Phase 1 `d4f11d6` adds `generate_cartesia(text, voice_id, output_path, language, model_id)` wrapping POST `api.cartesia.ai/tts/bytes` (Cartesia Sonic 2 REST; `X-API-Key` + `Cartesia-Version` headers; JSON body with `model_id`/`transcript`/`voice.id`/`output_format.container=mp3`/`language`); graceful failure returning False on missing key / HTTP error / timeout (never raises — caller fallback strategy). Phase 2 `cac8650` adds `_resolve_tts_provider(scene, character, settings)` — Korean detection via case-insensitive prefix match on `"ko"` (matches ISO `"ko"`, `"ko_KR"`, AND human-name `"Korean"` from `domain/language_defaults.py:66` `PIPELINE_LANGUAGE_DEFAULTS`); returns `"CARTESIA_SONIC_2"` for Korean + `CARTESIA_API_KEY` set, `"ELEVENLABS"` otherwise. Phase 3 `9ba2eb8` wires per-line dispatcher into `generate_dialogue_voiceover` PATH 2 loop with auto-fallback to ElevenLabs on Cartesia False return. `CARTESIA_API_KEY` added to .env cycle-15 entry by user-principal; verified loads via `settings.cartesia_api_key`. **Cost impact:** Korean dialogue shots ~$0.03/shot (ElevenLabs v3 Korean fallback) → ~$0.008/shot (Cartesia Sonic 2 native; `cost_tracker.py` `CARTESIA_SONIC_2` entry added this phase per descriptor `domain/scene_decomposer.py:67` `per_shot_cost: 0.008`); English/multilingual unchanged ($0.01/shot ElevenLabs path). Net: Korean reel-scale cost reduction (~75% savings on dialogue line) + native Korean prosody quality lift (~75ms latency vs ~3s ElevenLabs per descriptor). 27 new unit tests in `tests/unit/test_audio_dialogue_cartesia.py` covering 10 generate_cartesia + 13 router + 4 dispatcher-integration paths. PR-DIALOGUE cell extended with "Language-routed provider" subsection covering Cartesia/ElevenLabs split + new failure modes (Cartesia voice_id mismatch → fallback verification). Optional PA-TTS cell skipped (Tier D sweep can be added next cycle if needed; current cycle scope is impl + tests). **DRAFT v0.9.6** — director follow-up fold per user-principal direction (foley feature ship, cycle-15 entry): audio architecture **TRI-MIX extension** lands per 4-slice subagent-driven implementation. `audio/foley.py` re-added at `c15b2a8` (was deleted at `48f2a24` for zero callers, now wired with Stability AI Stable Audio 2.0 REST via `generate_stability_foley(foley_description, output_path, duration, ...)`); pipeline `_ensure_scene_foley(scene)` added at `89635b1` (`cinema_pipeline.py:562-608`, mirrors `_ensure_scene_audio` per-scene caching shape, aggregates unique `scene_foley` descriptors across `scene["shots"][*].scene_foley` populated by `domain/scene_decomposer.py:613` from LLM decomposition); 3-input ffmpeg amix lands at `78ffc43` (`cinema_pipeline.py:1254` `[voice][bgm][foley]amix=inputs=3:duration=first:dropout_transition=2[aout]`; voice=1.0 / bgm=0.12 / foley=0.20 volume balance per code-quality reviewer disposition); checkpoint persistence + concat-list quote escaping fix lands at `1b51ddb` (`cinema/checkpoint.py:100-101,172-173`; closes Lane V #3 C1 CRITICAL + I1 IMPORTANT). Brief originally pointed to `phase_c_ffmpeg.py` for the amix work (Rule #12 violation in original brief — actual amix lives in `cinema_pipeline.py::_assemble_final`; `phase_c_ffmpeg.py` has zero amix references, handles only video concat). **Cost impact:** +$0.15-0.30 per Tier C reel for foley (5-10 shots × $0.03 per `cost_tracker.py:62 STABILITY_FOLEY` entry added with the module). New PA-FOLEY cell added to §5 for Tier D parameter sweep (LLM-vibe-driven default vs operator-explicit foley prompts). 4 Lane V cycles ran across the 4-slice dispatch: T1 spec ✅ + 2 IMPORTANT + 3 MINOR folded into T2; T2 spec ✅ + 1 IMPORTANT dual-state finding folded into T3; T3 spec ✅ + 1 CRITICAL + 2 IMPORTANT, C1 + I1 closed in fix commit. **DRAFT v0.9.5** — operator follow-up fold per user-principal direction cycle-15 entry: PA-AUDIO cell scope EXTENDED from "Loudnorm targets" only → "BGM provider + Loudnorm targets" with new Set 3 (Suno V5 BGM provider sweep vs FAL Stable Audio Sets 1+2; same -23 LUFS cinema loudnorm target for provider isolation). `SUNO_API_KEY` added to .env this cycle (length 32; loads via `settings.suno_api_key`); pipeline `cinema_pipeline.py:538` still hardcoded to `generate_fal_bgm` for BGM default (Suno V5 is alternate per `.env.example:64` "alternate BGM provider, default is FAL Stable Audio") — PA-AUDIO Set 3 invokes `generate_suno_v5()` directly (`audio/music.py:140`) for A/B comparison without pipeline change. Cost moves $0 → ~$0.10-0.50 (Suno V5 pricing TBD per execution; not in `cost_tracker.py:API_COST_USD` — Set 3 execution becomes calibration measurement per Rule #1 ADR-013 falsifiability discipline; parallel to v0.9.2 PA-IDENTITY UNKNOWN-calibration framing). Cell count unchanged (still 7 PA-*; 30 total). **DRAFT v0.9.4** — operator follow-up fold per user-principal direction cycle-15 entry "use ltx api when possible" (symmetric to v0.9.3 Kling fold): PA-VIDEO Set 2 re-framed to make LTX Native preference explicit — `ltx_native.py:53-58` already implements native-first ("Prefer native LTX API — direct, no proxy"; `if self.ltx_key: self.mode = 'native'`); brief Set 2 updated with explicit native API endpoint `api.ltx.video/v1`, model identifier `ltx-2-3-pro` (verified at `ltx_native.py:233`), `cost_tracker.py:51` LTX rate ($0.10/5s), and FAL fallback path `fal-ai/ltx-2/image-to-video` (`ltx_native.py:29`) for native-key-unset case. **v0.9.3** — operator follow-up fold per user-principal direction cycle-15 entry "use kling api when possible": PA-VIDEO Set 3 re-routed from FAL Kling 3.0 (`phase_c_ffmpeg.py:508` cascade fallback) → KLING_NATIVE (`kling_native.py:KlingNativeAPI`; JWT HS256 auth at `api.klingai.com`); aligns brief with code reality (`phase_c_ffmpeg.py:127-130` cascade default already prefers KLING_NATIVE per "NATIVE API HANDLERS (priority — direct, no proxy, lower cost)" + `workflow_selector.py` defaults to KLING_NATIVE for most shot types). Cost estimate updated to ~$0.15-0.50/5s clip per `cost_tracker.py:45` KLING_NATIVE rate (replaces stale ~$0.40 FAL estimate). FAL Kling 3.0 remains cascade fallback only when KLING_NATIVE fails. **PA-VIDEO routing convergence: all 3 sets now native-preferred (Set 1 Veo Vertex native; Set 2 LTX native; Set 3 Kling native) with FAL as cascade fallback per `phase_c_ffmpeg.py:191` "NATIVE API HANDLERS (priority — direct, no proxy, lower cost)" comment.** PA-VIDEO cell-level cost adjusted ~$1.00 → ~$0.75-1.10. Total Tier B+C+D budget cap ($50 user-§9 Q-6) unchanged. **GEMINI_API_KEY added to .env this cycle** — unlocks P10/P11 3-model LLM ensemble (`[claude-sonnet-4, gpt-4o, gemini-2.5-pro]` tweak variant (c)) + cross-judge testing (P11 tweak variants (b) + (c)) per testplan §6.4. v0.9.2 extended director's v0.9.1 (`68c5cba`) by closing: (a) director F1 PA-IMAGE side ("outside production-tier cost envelope" callout to Set 2; symmetric to P-KEYFRAME parenthetical added at v0.9.1); (b) director F2 PA-IDENTITY pass-rate predictions replaced with HONEST hedge per option (i) ("UNKNOWN — calibration value"; preserves falsifiability discipline). Director F7 PA-IDENTITY field-naming clarity (identity_strictness vs identity_threshold) was already folded at v0.9.1 (`68c5cba`) per converged operator F-5/F-6 + director F7 finding; operator v0.9.2 + v0.9.3 leave intact. Folds joint v0.9 mid-prep review findings from both seats per operator REPLY §2 hybrid protocol. Director side (T10:46:03Z verification-report `fe26804`): 7 findings on 7 operator-authored PA-* cells (v0.9.1 closed F1 P-KEYFRAME-side + F7; v0.9.2 closes F1 PA-IMAGE-side + F2; F3-F6 NO ACTION per director option (c) recommendation). Operator side (T10:45:00Z verification-report `49c8af3`): 22 net findings on 23 director-authored P + G + PR cells (v0.9.1 closed all IMPORTANT + high-value MINOR per user Option A; ADR-013 P-* basis additions F-13/F-22 + cross-phase missing-mode predictions F-14/F-17/F-21 deferred to v0.9.X or advisory). Substantive state: **ALL 30 test cell PREDICTIONs filled** (9 P + 6 G + 8 PR + 7 PA); **ALL 9 user §9 questions ANSWERED**; impl-mismatch line refs corrected to HEAD (F-1 G-PERF `cinema_pipeline.py:944-961`; F-20 §6 matrix iterate path); unfalsifiable phrasing tightened where flagged (F-11 P-DECOMPOSE/P-CHIEFDIR/PR-MOTION/PR-CONTINUITY; F2 PA-IDENTITY pass-rate UNKNOWN-calibration); cross-cell contradictions closed (F-4 PR-STORY field; F-9 PR-CONTINUITY return shape; F-16 G-SCREEN failure-mode #2; F1 PA-IMAGE↔P-KEYFRAME cost range both sides; F-2 §5.5 G-PLAN field; F7 PA-IDENTITY field-naming). Operator REPLY folded at `a9b1c32` preserved; Candidate #8 filed at `1af3528` preserved.
- AWAITING: user-principal execution authorization signal (only remaining gate). Pre-flight A1-A9 all-green at pod `525nb9d5cc0p3y` per v1.0 SHIP entry above. Execution still synchronous joint-seat per Q9; first window when both seats available. Deferred-MINOR findings (operator F-13/F-14/F-17/F-21/F-22 + director F2-F6) remain advisory; NOT blocking execution per cycle-15-entry close conventions.
...
- **EXECUTABLE state at v1.0 SHIP** — §"Pre-flight checklist" all-green at pod `525nb9d5cc0p3y` ✅ + operator REPLY landed `a9b1c32` ✅ + brief v1.0 ✅ + user-principal execution authorization PENDING (only remaining gate).
...
3. **Tier C — Full reel real-generation walk-through (pod required, ~$3-7 + ~$0.15-0.30 foley).** Medium project: 1 scene, 3-5 shots, performance enabled, identity validation enforced. Validates inter-shot continuity, identity, audio mixing (now tri-mix per v0.9.6: voice + BGM + foley), gate flow. Foley cost: 5-10 shots × $0.03/scene per `cost_tracker.py:62 STABILITY_FOLEY` — graceful degradation to 2-input mix if `STABILITY_API_KEY` absent (zero foley cost).
...
for node in "PulidFluxInsightFaceLoader" "PulidFluxModelLoader" "ApplyPulidFlux" "PulidFluxEvaClipLoader" "ControlNetApply" "IPAdapter" "VAELoader" "CheckpointLoaderSimple" "FluxGuidance"; do
...
| P-ASSEMBLY | Final assembly | `_assemble_final` (FFmpeg stitch + color + tri-mix voice/BGM/foley + 2-pass loudnorm) | B + C | $0 (local FFmpeg; foley-gen cost belongs to PA-FOLEY / runtime) | **FILLED v0.2; extended v0.9.6 (foley tri-mix)** |
...
**Estimated cost:** $0.05-0.30 per shot via ComfyUI pod (model-dependent; SDXL ~$0.02, FLUX ~$0.10-0.20 per image at typical settings; **max-tier at Tier D PA-IMAGE Set 2 is ~$0.40 per `quality_max.py:705` documented 8× factor — outside production-tier cost envelope, only invoked when sweep-set or operator-explicit max-tier flag set**)
...
#### Test cell P-ASSEMBLY — Final assembly (FFmpeg stitch + color + tri-mix voice/BGM/foley + loudnorm)

**Phase / class:** Phase (step 19 in `generate()`)
**Stage in pipeline:** ARCHITECTURE §4.1 step 19 + §12 audio pipeline; impl at `cinema_pipeline.py:_assemble_final` + `phase_c_ffmpeg.py` (stitch/concat only — amix lives in `cinema_pipeline.py`; **note** original brief Rule #12 violation pointed to `phase_c_ffmpeg.py` for amix, v0.9.6 corrects: actual amix at `cinema_pipeline.py:1254`)
**Test tier:** B (single-shot assembled into 1-shot reel) + C (multi-shot reel)
**Estimated cost:** $0 (all local FFmpeg; foley GENERATION cost happens upstream during `_ensure_scene_foley`, see PA-FOLEY for the parameter sensitivity sweep)
**Wall-clock prediction:** 30s-2min for Tier B; 1-5min for Tier C (depends on reel length + transcode complexity + foley concat if multi-scene)

**PREDICTION (filled at v0.2 from impl-read at `cinema_pipeline.py:644-665` + `_assemble_approved_takes_core` doc per §8 protocol; extended v0.9.6 for tri-mix per `cinema_pipeline.py:_concat_foley_track` + `_assemble_final:1191-1280`):**

- **Expected output (shape):** `{"success": True, "final_path": <path>}` with file at `final_cinema.mp4` in project export_dir. Per-scene preview files at `scene_<id>_preview.mp4` from `generate_scene_preview` step. Final mp4 is single concatenated file with **tri-mix audio**: voice (from dialogue takes, baked into stitched clips) + BGM (project-wide, FAL/Suno) + foley (per-scene, Stability AI Stable Audio 2.0, concatenated end-to-end via `_concat_foley_track` when multi-scene). When `foley_audio_paths` is empty (STABILITY_API_KEY absent or all foley gen failed): falls back to legacy 2-input mix (voice + BGM only) — graceful degradation per `_assemble_final` foley-availability branch.
- **Expected content quality:** mp4 file exists + plays in standard player; duration ≈ sum(shot durations) + transitions; **audio layer count via `ffprobe -show_streams -select_streams a` = 1 stream (post-mix; tri-mix produces single output stream)**; BGM audible at -23 LUFS (broadcast standard) per loudnorm; **foley layer perceptibly present at 0.20 volume — ambient environmental beds (footsteps, room tone, weather per scene_foley descriptors) without overpowering dialogue (1.0) or BGM (0.12)**; no abrupt audio cuts at shot boundaries; color grade applied uniformly per scene (no scene-to-scene color jump unless intended); resolution matches `aspect_ratio` setting throughout. Subject identity preserved at scene boundaries (continuity from P-IDENTITY).
- **Expected latency:** 30s-2min for Tier B (1-3 shots × 5s each); 1-5min for Tier C (5+ shots × 5s + audio mix + 2-pass loudnorm). Two-pass loudnorm alone takes ~30-90s for typical reel length. Foley concat helper adds ~2-5s for multi-scene reels.
- **Expected cost:** $0 (local FFmpeg + local color grade preset + BGM already generated upstream + foley already generated per-scene upstream by `_ensure_scene_foley`). **Note**: foley GEN cost (~$0.15-0.30 per Tier C reel) is incurred in the upstream `_ensure_scene_foley` per-scene step, NOT in the assembly step itself. See PA-FOLEY for parameter sensitivity.
- **Expected failure modes (top 4 — extended v0.9.6):**
  ...
  4. **Foley concat fails or tri-mix amix=inputs=3 fails** (codec mismatch across per-scene foley clips; concat-list parse error; ffmpeg amix incompatible inputs) → graceful fallback to 2-input mix per `_assemble_final` exception cascade. Failure is SILENT in current impl (logs at warning level only); foley layer drops without user-facing error. **Adjustment**: surface a structured warning to lifecycle if foley was present but failed; check ffprobe stream count post-assembly to confirm foley intent honored.
- **Expected adjustment indicators:**
  ...
  - Foley fall-through to 2-input mix → check `_runstate.foley_audio_paths` content (was foley generated upstream?); verify `STABILITY_API_KEY` set; check temp_dir `foley_<scene_id>.mp3` cache file existence; verify checkpoint round-trip (`1b51ddb` fix) for resume scenarios
...
- **Expected content quality:** ≥3 shots per scene; each shot has a non-empty `description` field (≥20 chars, falsifiable) and non-empty `prompt` field; camera directives from canonical vocabulary (falsifiable: shot's `camera` value ∈ {zoom_in_slow, zoom_out_slow, pan_left, pan_right, dolly_back, dolly_forward, ...}); per-shot `characters_in_frame` is a subset of scene's `characters_present` (falsifiable: set-membership check); for a 60s scene, expect 5-10 shots. **Subjective quality signals (INSIGHT-only, NOT subject to PASS/FAIL):** "shots advance scene narrative" / "camera matches scene mood" — qualitative judgment recorded in INSIGHT not in DELTA classification.
...
- **Expected content quality:** mp3 plays in standard player; duration ≈ 47s (caller-fixed at `cinema_pipeline.py:538`; per ARCHITECTURE §12.5 "practical max for FAL Stable Audio"; FAL function default is 42s at `audio/music.py:240`; `generate_suno_v5` supports up to 240s at line 164 — so 47s is caller-policy, NOT a FAL hard-cap); music style matches `music_mood` setting (default "suspense"); mastering produces louder + more dynamic output vs raw generation (cinema_master preset target ~-14 LUFS for streaming). Non-empty audio (rejects silent-track failure mode).
...
- **Expected content quality:** decisions reflect quality of decompose output. Per-decision shape: APPROVED returns shots unchanged; MODIFIED returns full modified shot list (falsifiable: compare modified vs original shot count + content delta); REJECTED returns violations list (falsifiable: non-empty list with structured reason). **Distribution is calibration-pending:** brief author has no prior baseline run data; "APPROVED ~60-80% / REJECTED <10%" are first-execution hypotheses, NOT predictions grounded in measured distribution. After N=10+ baseline runs, distribution becomes the actual prediction; until then, the per-decision-shape checks above are the falsifiable signals.
...
**Stage in pipeline:** ARCHITECTURE §6.1 PERFORMANCE_REVIEW predicate (three-paths-satisfied) at `cinema/review/controller.py:223-234`; approve endpoint per ARCHITECTURE §3.6 at `web_server.py:1629 api_approve_performance_take`; `all_skipped` short-circuit at `cinema_pipeline.py:944-961` (v0.9.1 line-ref correction per operator joint-review F-1; was 767-788 at v0.7 inherited from ARCHITECTURE.md drift; verified at HEAD via `grep -n "PERFORMANCE_SKIPPED_GATE\\|all_skipped" cinema_pipeline.py` → 944/950/961; redundant per ARCHITECTURE §6.1 note but preserved for explicit `PERFORMANCE_SKIPPED_GATE` UX event); auto-approve gated behind `CINEMA_AUTO_APPROVE_MOTION` opt-in per ARCHITECTURE §7.7.2 + ADR-014
**Test tier:** C only
...
**PREDICTION (filled at v0.7; line refs corrected at v0.9.1 per operator F-1 — impl-read at `cinema/review/controller.py:223-234`, `cinema_pipeline.py:951-955`, `cinema_pipeline.py:944-961` (was 767-788; ARCHITECTURE-drift inheritance), ARCHITECTURE §6.1, per §8 protocol):**
...
- **Expected latency:** 500ms-bounded as above. The orchestrator's `all_skipped` shortcut at `cinema_pipeline.py:944-961` emits `PERFORMANCE_SKIPPED_GATE` event instantly when ALL shots route to SKIP (no operator action needed); this is a UX optimization, redundant with the predicate's path-(a).
- **Expected cost:** $0 for gate.
- **Expected failure modes (top 3):**
  1. **SKIP shot still blocks gate** — `performance_engine` field has stray whitespace that survives `(performance_engine or "").upper() == "SKIP"` literal compare (case-variation is handled by `.upper()` per controller.py:229-234, so NOT a failure mode; v0.9.1 reframe per operator F-15). Risk concentrated at the 3 write-paths in `cinema/shots/controller.py:618/655/736` (verified via `grep -n 'performance_engine.*SKIP' cinema/shots/controller.py`) — any of those producing non-canonical-literal `"SKIP"` value (e.g., trailing whitespace, mixed-case BEFORE `.upper()` normalization downstream) propagates the bug. Symptom: pure-SKIP reel doesn't fast-forward; operator confused why gate is paused with no approvable takes.
...
- **Expected adjustment indicators:**
  - SKIP still blocks → verify ALL 3 write-sites at `cinema/shots/controller.py:618/655/736` use canonical literal `"SKIP"` (no whitespace, no concatenation); re-grep predicate at `controller.py:229-234`; trace `cinema/performance/_router.dispatch` writes for canonical literal hygiene (v0.9.1 reframe per operator F-15)
...
- **Expected failure modes (top 3):**
  ...
  2. **UI surfaces approve button when flag is off** — frontend bug: `CINEMA_SCREENING_STAGE=0` opt-out is DESIGN-INTENDED backend behavior (worker skips screening; pipeline completes without gate) but UI still shows the screening approve button + iterate-from-screening UI, confusing operator. Backend gate IS correct (idempotent no-op flag write); failure is frontend feature-detection. (v0.9.1 reframe per operator F-16; the opt-out itself is intentional behavior, not a gate failure.)
...
- **Expected adjustment indicators:**
  ...
  - UI surfaces approve when flag-off → expose `CINEMA_SCREENING_STAGE` state via `/api/feature-flags` endpoint OR build-time inject so UI feature-detects; audit `ScreeningStage.tsx` for flag-aware rendering (v0.9.1 reframe per operator F-16)
...
- **Expected failure modes (top 3):**
  ...
  2. **Character coverage gap** — shots' per-shot `characters_in_frame` field omits character_ids listed in scene's `characters_present` field (note distinct field-level scoping: `characters_present` is SCENE-level, `characters_in_frame` is per-SHOT — v0.9.1 correction per operator F-4); root cause: prompt didn't enforce per-character minimum or list characters prominently
...
- **Expected adjustment indicators:**
  ...
  - Character coverage gap → add "every character_id in scene's `characters_present` appears in ≥1 shot's `characters_in_frame`" constraint; add post-validation set-membership check (falsifiable: `set(union(shot.characters_in_frame for shot in shots)) ⊇ set(scene.characters_present)`)
...
- **Expected content quality:** prompt contains camera token from canonical list (falsifiable: `shot.get("camera")` value substring present in encoded motion prompt — regex/substring match); character descriptor from PR-IMAGE present in motion prompt (falsifiable: shot's character name + visual descriptor present, same hash as PR-IMAGE prompt-fragment for that character); engine-appropriate format selected (falsifiable: prompt encoding shape matches engine class — Veo prompt has natural-language sentences, LTX prompt has comma-separated keywords). **Subjective signal (INSIGHT-only, NOT classified):** "style continuity from keyframe to motion" — qualitative; recorded in INSIGHT not DELTA.
- **Expected failure modes (top 3):**
...
- **Expected output (shape):** dialogue strings per shot/scene with speaker attribution + content; LLM call returns structured JSON via tools-appended format (line 94); each dialogue entry maps to a character_id in the scene's `characters_present` set (scene-level field; per-shot `characters_in_frame` may narrow which characters speak in which shot).
...
- **Expected adjustment indicators:**
  ...
  - Language drift → add per-language sub-prompt or explicit "preserve source language's structural conventions (subject ordering, honorifics, register)" instruction; route per-character `language_pref` field **(proposed; future model-change required — field does not exist at HEAD per operator F-8)**
  - Voice collapse → introduce per-character voice anchor in `user_prompt` (each character's idiomatic snippet); enforce voice-distinctness post-check; expose `character_voice` trait at character-creation time **(proposed; future model-change required — field does not exist at HEAD per operator F-8)**

**Language-routed provider (added v0.9.7 per Cartesia Sonic 2 re-introduction at `d4f11d6`/`cac8650`/`9ba2eb8`):**

Dialogue voiceover provider selection is now language-routed at the per-line dispatcher level inside `generate_dialogue_voiceover` (PATH 2 per-line loop at `audio/dialogue.py`). Routing decided by `_resolve_tts_provider(scene, character, settings)`:

- **Korean dialogue** (project `global_settings.language == "Korean"` OR explicit ISO `"ko"`/`"ko_KR"` on scene/character) → routes to `generate_cartesia` (Cartesia Sonic 2 REST; native Korean prosody, ~75ms latency, ~$0.008/shot). Requires `CARTESIA_API_KEY` env (added cycle-15 entry).
- **All other languages** (English / Japanese / Mandarin / etc.) → routes to existing ElevenLabs `client.text_to_speech.convert` path (multilingual; unchanged behavior).
- **Cartesia-key-missing fallback** — if `CARTESIA_API_KEY` is unset, Korean still routes to ElevenLabs (graceful degradation; never breaks pipeline on missing credentials).
- **Cartesia-API-failure fallback** — if `generate_cartesia` returns False (HTTP error / timeout / format issue / etc.), the per-line loop falls through to the ElevenLabs branch for that specific line. Per-line atomicity: one Korean line failing Cartesia does NOT prevent the remaining Korean lines from attempting Cartesia.

**New failure modes (Tier C/D verification surface):**

1. **Cartesia voice_id schema mismatch** — Cartesia voice IDs are UUID-shaped (`abc-123-...`), ElevenLabs IDs are 20-char alphanumeric (`pNInz6obpgDQGcFmaJgB`). If a project's character record holds an ElevenLabs voice_id but routes to Cartesia, Cartesia returns 4xx "voice not found." Expected behavior: Cartesia returns False, fallback to ElevenLabs for that line (per-line atomic). Verification: Tier C reel with mixed Korean characters; observe `[CARTESIA] failed: ...; falling back to ElevenLabs` log line; observe ElevenLabs line generation count matching the fallback count.
2. **Language string drift** — project's `global_settings.language` value depends on UI/defaults; values like `"한국어"` (Korean name in Korean) or `"kr"` (incorrect — actual ISO is `"ko"`) won't match the `"ko"` prefix rule. Expected: such values route to ElevenLabs (default). Verification: probe `_resolve_tts_provider` with the actual project's language string before Tier C execution.
3. **Cost-tracker integration** (v0.9.8 fix — operator-side I-2 closure from cycle-15 code-quality review) — dispatcher Cartesia success branch at `audio/dialogue.py:357-376` now calls `cost_tracker.CostTracker().record_api_call("CARTESIA_SONIC_2", operation="dialogue_tts")` per-line on each successful Cartesia generation. Cost-record failure is best-effort wrapped (mirrors `cinema/shots/controller.py:551-554` pattern) — if CostTracker constructor fails (SQLite locked, etc.) the TTS success itself is not regressed. **Asymmetry note (pre-existing):** ElevenLabs path remains untracked — no entry in `cost_tracker.py:API_COST_USD`, no `record_api_call("ELEVENLABS"...)` callers anywhere in the codebase. Adding symmetric ElevenLabs tracking is a future small-scope improvement (v0.9.X+). Verification: post-Tier-C check observed Korean dialogue spend equals `(N_korean_lines × $0.008)` in cost-tracker SQLite; observed English dialogue spend remains $0 in cost-tracker (pre-existing dormant; reconcile manually against ElevenLabs invoice if needed).

**Adjustment indicators:**

- Voice_id mismatch → expose Cartesia-voice-pool selection in UI's voice picker when project language is Korean; auto-assign Cartesia voices for new Korean characters (parallel to `default_male_voice` / `default_female_voice` at `domain/language_defaults.py:81-82` Korean block).
- Language string drift → normalize project language to canonical form (`"Korean"` or `"ko"`) at write-time in settings UI; document the expected values in `domain/language_defaults.py` schema.
- Cost-tracker integration → CLOSED at v0.9.8 (operator I-2 fix). Future: symmetric ElevenLabs tracking + per-line `shot_id` propagation for granular budget attribution (current `operation="dialogue_tts"` is line-grain; shot-grain would require dispatcher to thread shot_id through `generate_dialogue_voiceover`).
...
- **Expected output (shape):** single `enhanced` dict (NOT tuple — v0.9.1 correction per operator F-9; impl at `domain/continuity_engine.py:454` returns one dict with mutated `prompt` field + added `continuity_config` field; original shot fields preserved). The mutated `prompt` field is `prompt_parts` joined — 6 fragment classes appended in order: raw shot prompt → location persistence → character identity fragments (with `left/center/right` spatial positioning when ≥2 characters present) → physics constraints → motion constraints → continuity notes. The added `continuity_config` field holds `anchor_image` path (or None) — anchor is NOT part of positive-prompt assembly per ARCHITECTURE §8.
- **Expected content quality:** all 6 fragment classes present when applicable (falsifiable: parse `prompt_parts` boundaries; expect non-empty fragments where input data supports them); spatial-positioning labels present in prompt when ≥2 chars (falsifiable: regex `/(left|center|right)/i` in prompt with multi-char input); physics constraints reflect scene's physical state; motion constraints reflect previous shot's action vector when `previous_shot` is provided; continuity notes from `shot["continuity_constraints"]` field; anchor_image resolves to a real file or None (falsifiable: `os.path.exists` check). **Subjective signal (INSIGHT-only):** "physics fragment matches scene reality" — recorded but not classified.
- **Expected failure modes (top 3):**
...
- **Expected adjustment indicators:**
  - Scene-context → add optional `scene_stage` parameter (intro/build/climax) with tempo-modulation hint OR composition between 2 vibes (transition mapping); tweak-variant: surface scene-stage to operator at BGM-prompt time **(API change — `_build_music_prompt` signature would expand; caller-site updates required at `audio/music.py:158/235` per operator F-19)**
...
|---|---|
| **G-PLAN** | `jq '.scenes[].shots[] \| {id, plan_status}' projects/<pid>/project.json` (v0.9.1 correction per operator F-2 — field is `plan_status` string ∈ {"approved", "rejected", "pending_review"}; the prior `plan_approved` was a non-existent field returning null) |
...
- **Sweep sets:**
  ...
  - **Set 2 (max-tier):** `quality_max.generate_ai_broll_max` with `pulid_max.json` + SUPIR + N=8 best-of + HiDream-I1 optional swap. ~4 min; ~$0.40. **Cost callout (v0.9.2 per operator F1 PA-IMAGE-side fold):** Set 2's $0.40 explicitly exceeds P-KEYFRAME's production-tier cost envelope ($0.05-0.30 upper bound) per `quality_max.py:705` documented 8× factor. Only invoked when PA-* sweep-set OR operator-explicit max-tier flag set; NOT in P-KEYFRAME production path. P-KEYFRAME cost header at `68c5cba` v0.9.1 fold added the corresponding parenthetical pointing back to PA-IMAGE Set 2.
...
**Test tier:** D
**Estimated cost:** ~$0.75-1.10 (3 engines × 1 shot: Veo native Vertex ~$0.50 + LTX native ~$0.10 + Kling Native ~$0.15-0.50 per `cost_tracker.py:45` KLING_NATIVE rate; range reflects per-clip variance — short clip ≈ $0.15, full 5s ≈ $0.50)
...
- **Sweep engines:**
  ...
  - **Set 2 — LTX Native (`ltx_native.py:LTXVideoAPI`):** `model="ltx-2-3-pro"` via **native API** at `api.ltx.video/v1` (line 233; LTX_API_KEY); **preferred per code default** in `ltx_native.py:53-58` ("Prefer native LTX API — direct, no proxy"; `if self.ltx_key: self.mode = 'native'`) + per user-principal direction cycle-15 entry "use ltx api when possible"; ~$0.10/5s clip per `cost_tracker.py:51` LTX rate; ~30-60s latency. FAL proxy (`FAL_MODEL_ID = "fal-ai/ltx-2/image-to-video"` at line 29) is cascade fallback only when LTX_API_KEY unset.
  - **Set 3 — Kling Native (`kling_native.py:KlingNativeAPI`):** Kling 3.0 via **native API** (KLING_ACCESS_KEY + KLING_SECRET_KEY JWT HS256 auth at `api.klingai.com`); **preferred per code cascade default** in `phase_c_ffmpeg.py:127-130` ("NATIVE API HANDLERS (priority — direct, no proxy, lower cost)") + per user-principal direction cycle-15 entry "use kling api when possible"; ~$0.15-0.50/5s clip per `cost_tracker.py:45` KLING_NATIVE rate; ~45-120s latency. FAL Kling 3.0 (`phase_c_ffmpeg.py:508`) is cascade fallback only when KLING_NATIVE fails or KLING_ACCESS_KEY/KLING_SECRET_KEY unset.
...
- **Expected cost:** ~$0.75-1.10 total per `API_COST_USD` + `cost_tracker.py:45` KLING_NATIVE rate. Veo dominates; LTX cheapest; Kling Native variance ~$0.15-0.50.
- **Expected failure modes (top 3):**
...
- **Sweep thresholds:**
  - **Set 1 (lenient):** threshold = 0.60. Predict: **UNKNOWN — calibration value, see note below.**
  - **Set 2 (default):** threshold = 0.70. Predict: **UNKNOWN — calibration value, see note below.**
  - **Set 3 (strict):** threshold = 0.80. Predict: **UNKNOWN — calibration value, see note below.**
  - **Pass-rate prediction calibration note (v0.9.2 per operator F2 + director F2 + operator F-7 convergent fold per option (i) HONEST hedge):** Per Rule #1 (ADR-013) verification discipline — predicted pass rates are **UNKNOWN at v0.9.2 commit time**. No prior calibration data, no `domain/projects/` historical GhostFaceNet score-distribution analysis cited. Prior v0.6/v0.9 "~80-90% / ~60-75% / ~30-50%" predictions were best-guess intuitions, not grounded in observed data; removed to preserve falsifiability. **PA-IDENTITY's first execution is calibration**: pass-rate distributions emerge from observed GhostFaceNet score distribution on the sample project. Future sweeps will narrow predictions based on baseline measurement. **NO PREDICTED VALUES set this commit** — avoids post-observation retroactive rationalization ("lenient was lenient, strict was strict") that unfalsifiable predictions invite. Falsifiability discipline: predictions must be capable of failing; UNKNOWN is the honest hedge.
...
- **ADR-013 quantitative basis:** identity_score range [0.0-1.0] per `phase_c_vision` validator API. **Two distinct fields exist** (v0.9.1 clarification per operator F-5 + F-6 + director F7): `identity_strictness` (project-wide override at `domain/project_manager.py:322`, **default 0.60**) and `identity_threshold` (per-shot fallback at `cinema/shots/controller.py:488 cc.get("identity_threshold", 0.70)`, **default 0.70**). Logic at `controller.py:490-491`: strictness wins if set, else threshold. **This sweep operates on `identity_strictness`** (the operator-knob override; P-IDENTITY adjustment indicators also use `identity_strictness`). Sweep values 0.60/0.70/0.80 deliberately bracket both defaults; Set 2=0.70 collides with the per-shot fallback default (operator F-7 noted; intentional bracket-the-default design — alternative widen-sweep to 0.55/0.70/0.85 deferred to v0.9.2 if execution surfaces calibration need).

**ACTUAL / DELTA / INSIGHT / ADJUSTMENT:** filled during execution per §1.3 cell-artifact convention.

#### Test cell PA-AUDIO — BGM provider + Loudnorm targets (FAL vs Suno V5; cinema -23 LUFS vs streaming -16 LUFS)

**Phase / class:** Parameter sensitivity (BGM provider routing + ffmpeg two-pass loudnorm targets; PA-* class)
**Stage in pipeline:** ARCHITECTURE §12 audio pipeline; impl at `audio/music.py:88 _build_music_prompt` + `audio/music.py:140 generate_suno_v5` + `audio/music.py:240 generate_fal_bgm` + `phase_c_ffmpeg.py:1000 two_pass_loudnorm`
**Test tier:** D
**Estimated cost:** **~$0.10-0.50** (Set 1+2 FAL Stable Audio + local ffmpeg ≈ $0; Set 3 Suno V5 ~$0.10-0.50 per 47s call, pricing TBD per execution — Suno V5 cost not in `cost_tracker.py:API_COST_USD`)
**Wall-clock prediction:** 5-15 minutes (3 × full-reel loudnorm passes + 1 × Suno V5 generation ~30-60s)

**PREDICTION (filled v0.6 from testplan §6.7 ffmpeg loudnorm + `phase_c_ffmpeg.py:1000-1060`; extended v0.9.5 with Suno V5 provider sweep per user-principal direction cycle-15 entry + `SUNO_API_KEY` added to .env):**

- **Expected output (shape):** 3 reel mp4 files — same scene assembly, different (BGM provider × loudnorm target). Each output has measured I (Integrated loudness LUFS) + LRA (Loudness range LU) + TP (True peak dBTP). Verifiable via `ffmpeg ... -af loudnorm=...:print_format=summary -f null - 2>&1`. Set 3 also has Suno V5-generated BGM track at `temp_dir/bgm_<vibe>_suno.mp3` (separate from FAL Stable Audio `bgm_<vibe>.mp3`) — direct A/B listening comparison possible.
- **Sweep sets:**
  - **Set 1 (FAL cinema):** BGM via FAL Stable Audio (current pipeline default per `cinema_pipeline.py:538 generate_fal_bgm(music_mood, bgm_path, duration=47)`); loudnorm target I=-23 LUFS / LRA=14 / TP=-2 dBTP (EBU R128 broadcast).
  - **Set 2 (FAL streaming):** Same BGM as Set 1; loudnorm target I=-16 LUFS / LRA=8 / TP=-1 dBTP (Spotify/YouTube-style louder + tighter range). Predict: perceptibly louder; less dynamic range.
  - **Set 3 (Suno V5 cinema) — NEW v0.9.5:** BGM via `generate_suno_v5(music_vibe, output_filename, duration=47, custom_lyrics="")` (`audio/music.py:140`); same loudnorm target as Set 1 (-23 LUFS cinema) for provider isolation. Suno V5 supports up to 240s per `audio/music.py:164` `min(duration, 240)` but constrained to 47s here for direct comparison with FAL. **Vocal mode behavior:** `instrumental=True` default (BGM use case); if `custom_lyrics` non-empty, Suno generates vocal track. Predict: Suno V5 produces richer / more song-like output vs FAL's ambient texture; potentially harder to mix-under-dialogue if vocals leak.
- **Expected content quality:** all 3 mp4s play; measured I within ±0.5 LUFS of target (two-pass loudnorm accuracy). Perceptual loudness difference Set 1 vs Set 2: ~7 dB louder (perceptible doubling). Perceptual BGM character difference Set 1 vs Set 3: FAL = ambient/textural; Suno V5 = structured song-form (intro/verse/outro arc) even at instrumental mode. Vocal-leak check: Suno V5 at `instrumental=True` should be vocal-free; verify via spectrogram or listening.
- **Expected latency:** Set 1+2 ~2-5 min per reel for two-pass loudnorm; Set 3 adds ~30-60s for Suno V5 generation + 2-5 min loudnorm. Total ~5-15 min sequential.
- **Expected cost:** Set 1+2 ~$0 (FAL Stable Audio already cached from pipeline run + local ffmpeg). Set 3 ~$0.10-0.50 (Suno V5 per-call pricing TBD per execution; Suno doesn't appear in `cost_tracker.py:API_COST_USD` — calibration value).
- **Expected failure modes (top 4):**
  ...
  4. **Suno V5 generation failure / timeout** — `audio/music.py:202` `[SUNO V5] timed out after Ns` or HTTP error; falls through to FAL Stable Audio per `generate_bgm()` smart-router behavior (but Set 3 invokes `generate_suno_v5()` directly so failure = Set 3 missing, not silent FAL substitution). Verify via console logs `[SUNO V5] ...`.
- **Expected adjustment indicators:**
  ...
  - Suno V5 vocal-leak at `instrumental=True` → file Suno API issue; switch default to FAL for BGM-under-dialogue use cases; document in PR-AUDIO-VIBE expected output
  - Suno V5 perceptibly worse than FAL for vibe X → consider per-vibe routing (e.g., orchestral cinema vibes → Suno; ambient/textural → FAL) instead of provider-wide default
  - Suno V5 cost exceeds estimate → adjust `cost_tracker.py:API_COST_USD` entry; possibly downgrade Suno path priority
- **ADR-013 quantitative basis:** EBU R128 spec for Set 1; common streaming-platform spec for Set 2; ±0.5 LUFS accuracy from libloudnorm documentation. Suno V5 cost is UNKNOWN at v0.9.5 commit time (`cost_tracker.py` has no SUNO_V5 entry; pricing varies by Suno subscription tier); first PA-AUDIO Set 3 execution becomes the calibration measurement — future PA-AUDIO sweeps cite measured cost.

**ACTUAL / DELTA / INSIGHT / ADJUSTMENT:** filled during execution per §1.3 cell-artifact convention.

#### Test cell PA-FOLEY — Foley prompt source + volume balance (vibe-driven vs operator-explicit; default 0.20 vs alternatives)

**Phase / class:** Parameter sensitivity (foley descriptor source + tri-mix volume; PA-* class; introduced v0.9.6 per audio architecture extension)
**Stage in pipeline:** ARCHITECTURE §12 audio pipeline (foley sub-layer); impl at `audio/foley.py:29 _build_foley_prompt` + `audio/foley.py:109 generate_stability_foley` + `cinema_pipeline.py:562 _ensure_scene_foley` + `cinema_pipeline.py:1083 _concat_foley_track` + `cinema_pipeline.py:1254` tri-mix amix
**Test tier:** D
**Estimated cost:** **~$0.15-0.30** per reel (5-10 shots × Stability AI Stable Audio 2.0 `cost_tracker.py:62 STABILITY_FOLEY = $0.03` per per-scene call; concrete sweep is 2 sets × ~$0.15-0.30 ≈ $0.30-0.60 total)
**Wall-clock prediction:** 5-12 minutes (2 × full-reel re-assembly + 2 × per-scene foley generation; Stability AI Stable Audio latency ~20s per scene per `domain/scene_decomposer.py:83 latency_s`)

**PREDICTION (filled v0.9.6 from impl-read at `audio/foley.py:29-103` + `cinema_pipeline.py:562-608,1083-1110,1244-1280`, per §8 protocol):**

- **Expected output (shape):** 2 reel mp4 files (same Tier C scene assembly, different foley descriptor source × volume balance). Each output has measured foley layer presence via `ffprobe -show_streams -select_streams a` (1 mixed audio stream per output; tri-mix amix produces single output regardless of input count). Per-scene foley clips cached at `<temp_dir>/foley_<scene_id>.mp3`; multi-scene reels also produce `<temp_dir>/foley_track.mp3` (concat'd). Stable Audio 2.0 returns raw mp3 bytes per `audio/foley.py:155-160`.
- **Sweep sets:**
  - **Set 1 (vibe-driven default; v0.9.6 default behavior):** foley descriptors come from `domain/scene_decomposer.py:613 shot.get("scene_foley", "ambient room tone")` (LLM-populated during decompose; falls back to "ambient room tone"). `_ensure_scene_foley` aggregates unique non-empty descriptors across scene's shots (`cinema_pipeline.py:573-580 dedup loop`), comma-joins, passes through `_build_foley_prompt` which maps to a 15-entry vibe dict (`audio/foley.py:29-82 env_prompts`: rain, crowd, traffic, machinery, footsteps, room_tone, etc.) or falls through with the raw string. Volume: 0.20 (default per `cinema_pipeline.py:1253`).
  - **Set 2 (operator-explicit override):** operator pre-edits each scene's `scene_foley` field via `web_server.py:1897` API endpoint to override LLM-populated values with explicit descriptors (e.g., scene 1 = "heavy rain on tin roof with distant thunder, dripping water in foreground", scene 2 = "crowded train station, footsteps on marble, distant announcements"). Same default volume 0.20. Predict: more cinematic / scene-specific environmental presence vs Set 1's vibe-mapped default; reveals whether LLM scene_foley generation quality matches operator intuition.
  - **(Optional) Set 3 (volume balance sweep):** same descriptor source as Set 1, but with `[2:a]volume=0.30[foley]` (louder foley); reveals whether default 0.20 is too quiet for environmental presence. Reserved for v0.9.7+ if Set 1+2 surface ambiguity.
- **Expected content quality:** all reel mp4s play; **foley layer audibly distinct** in Set 2 (operator-explicit) vs Set 1 (vibe-mapped) — Set 2 should reveal scene-specific environmental beds where Set 1 may produce generic ambience. Per-scene foley clips have duration ~5s × num_shots capped at 60s (per `_ensure_scene_foley` duration proxy at `cinema_pipeline.py:584`); shorter foley clips trim cleanly via `amix duration=first` clamp; longer clips concat sequentially in scene order. **Temporal alignment caveat (v0.9.6 design choice):** concat'd foley plays from t=0 of the final video sequentially, NOT time-aligned to scene boundaries — acceptable for "environmental beds" (ambient room tone, weather, crowd), perceptually wrong for "event-triggered SFX" (footstep on impact, door slam). PA-FOLEY's scope is environmental-only per `domain/scene_decomposer.py:83 description` ("long environmental beds (rain, crowd, machinery)"); future event-SFX support is out of v0.9.6 scope.
- **Expected latency:** Set 1+2 ~20s per scene × 3-5 scenes = 60-100s foley generation + 30s-2min concat + 1-5min re-assembly + loudnorm. Total ~3-8 min per set × 2 = ~6-16 min sequential.
- **Expected cost:** Set 1+2 each ~$0.15-0.30 per reel ($0.03 × ~5-10 scene-foley calls per reel). Total sweep ~$0.30-0.60. Stable Audio 2.0 pricing per `cost_tracker.py:62 STABILITY_FOLEY = $0.03` (calibrated against `domain/scene_decomposer.py:83 per_shot_cost`). First PA-FOLEY execution becomes calibration measurement if actual cost diverges from estimate; updates `cost_tracker.py` entry.
- **Expected failure modes (top 4):**
  1. **STABILITY_API_KEY absent or rate-limited** → `generate_stability_foley` returns None → `_ensure_scene_foley` returns "" → `foley_audio_paths` empty → graceful fallback to 2-input mix per `_assemble_final` foley-availability branch. **Set 1 + Set 2 BOTH fall back identically** — no observable foley layer in either output. Distinguishable via `[FOLEY] STABILITY_API_KEY not set; skipping` log line OR `[FOLEY] HTTP 4XX: ...` rate-limit log.
  2. **Foley codec mismatch across scenes** (Stable Audio returns inconsistent codec/sample-rate; concat-demuxer `-c copy` produces corrupt foley_track.mp3) → exception caught at `_concat_foley_track:1109`, returns None, falls back to 2-input mix. **Currently undocumented assumption** (Lane V #3 I2 deferred): Stability AI returns MP3 today; assumption holds for v0.9.6 ship but will need revisit when an alt foley provider lands.
  3. **Foley volume too loud or too quiet** — perceptual judgment, recorded in INSIGHT not DELTA. Adjustment: tune `[2:a]volume=0.20` (range 0.15-0.30 reasonable; >0.40 starts competing with dialogue at 1.0; <0.10 inaudible under BGM 0.12).
  4. **LLM scene_foley quality drift** (Set 1) — `domain/scene_decomposer.py` produces generic / unhelpful scene_foley descriptors (e.g., all scenes get "ambient room tone" regardless of context). Adjustment: tighten decomposer prompt at `domain/scene_decomposer.py:433-441` for richer environmental specificity; add few-shot examples.
- **Expected adjustment indicators:**
  - STABILITY_API_KEY missing → check `.env` cycle-15 entry; verify `settings.stability_api_key` loads (`config/settings.py:114`); validate API key against Stability AI dashboard
  - Codec mismatch → switch `_concat_foley_track` from `-c copy` to `-c:a libmp3lame` (transcode to homogeneous MP3; Lane V #3 I2 deferred fix); tradeoff: ~1-2s extra wall-clock for transcode vs robustness
  - Volume tuning → adjust `cinema_pipeline.py:1253` `[2:a]volume=0.20[foley]` value; document choice in commit body
  - LLM scene_foley generic → strengthen decomposer prompt; consider operator-explicit override (Set 2 becomes default); OR add post-decompose `_build_foley_prompt` enrichment step that adds details to generic descriptors
- **ADR-013 quantitative basis:** Stability AI Stable Audio 2.0 spec per `domain/scene_decomposer.py:83` (`per_shot_cost: 0.03, quality_score: 0.78, latency_s: 20, status: "live"`); 190s hard cap on duration per `audio/foley.py:155 min(int(round(duration)), 190)`; volume balance 0.20 chosen mid-range of 0.18-0.22 per Lane V #3 reviewer guidance (BGM 0.12 is non-diegetic ambient layer; foley 0.20 carries diegetic information per cinema audio mixing convention). v0.9.6 first execution becomes calibration measurement for both Stability AI cost and perceptual volume balance.
...
| Assembly: foley layer silently absent | `STABILITY_API_KEY` missing OR foley gen failed OR concat codec mismatch | `cinema_pipeline.py:_ensure_scene_foley` + `_concat_foley_track:1083-1110` + `audio/foley.py:generate_stability_foley` (folded v0.9.6 per Lane V #3 I2 disposition) | Check `[FOLEY] ...` log lines; verify `_runstate.foley_audio_paths` non-empty; if codec mismatch — switch concat to `-c:a libmp3lame` transcode; for resume scenarios verify checkpoint round-trip at `cinema/checkpoint.py:100-101,172-173` (fix `1b51ddb`) |
| Assembly: foley volume off-target | `[2:a]volume=0.20[foley]` filter param | `cinema_pipeline.py:1253` tri-mix amix filter chain (folded v0.9.6) | Adjust volume value (default 0.20; reasonable range 0.15-0.30; >0.40 competes with dialogue at 1.0; <0.10 inaudible under BGM 0.12) |
| Assembly: foley descriptors generic / unhelpful | LLM scene_foley quality drift | `domain/scene_decomposer.py:433-441` prompt + `audio/foley.py:_build_foley_prompt` 15-entry vibe dict | Strengthen decomposer prompt with environmental-specificity examples; OR rely on operator-explicit override via `web_server.py:1897 scene_foley` API; OR expand `_build_foley_prompt` vibe dict (rain/crowd/footsteps/etc.) |
...
| Surface A iterate: ignored | iterate plumbing OR feature flag | `web_server.py:1677 api_iterate_take` + `cinema/shots/controller.py regenerate_with_intent` + `CINEMA_DIRECTORIAL_ITERATION` env (v0.9.1 path correction per operator F-20 — no `cinema/iteration/` subdir exists; impl lives in web_server endpoint + ShotController method) | Verify flag; trace iterate endpoint to controller |
