...
# Create dummy inputs
subprocess.run("ffmpeg -y -f lavfi -i color=c=black -t 1 video.mp4", shell=True, stderr=subprocess.DEVNULL)
subprocess.run("ffmpeg -y -f lavfi -i aevalsrc=0 -t 1 tts.m4a", shell=True, stderr=subprocess.DEVNULL)
subprocess.run("ffmpeg -y -f lavfi -i aevalsrc=0 -t 1 bgm.m4a", shell=True, stderr=subprocess.DEVNULL)
