...
_threeway_state_line() {
  local py=".venv/bin/python"
  if [ -x "$py" ] && [ -f "scripts/threeway_readiness.py" ]; then
    "$py" scripts/threeway_readiness.py --state-line 2>/dev/null \
      || echo "threeway: ready-not-live unavailable (hook failed open)"
  else
    echo "threeway: ready-not-live unavailable (hook failed open)"
  fi
}
...
THREEWAY_READY=$(_threeway_state_line || echo "threeway: ready-not-live unavailable (hook failed open)")
...
- **Pytest:** ${PYTEST_LINE}
- **Threeway readiness:** ${THREEWAY_READY}
