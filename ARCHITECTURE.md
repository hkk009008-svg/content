...
| Shot work | [cinema/shots/controller.py](cinema/shots/controller.py) (2354 LOC) | `generate_keyframe_take`, `generate_performance_take`, `generate_motion_take`, `regenerate_shot`, `restart_shot`, `diagnose_clip`, `apply_correction`, `generate_scene_preview`. |
...
**2664 LOC, 66 `@app.route` decorators, 65 view functions** (one handler binds
two URLs — `/assemble` + `/proceed-assembly`, [web_server.py:2172-2173](web_server.py:2172)).
...
| `_lora_training_threads` | `_lora_training_lock` | [web_server.py:733-734](web_server.py:733) |
...
- `GET /api/projects/<pid>/stream` opens an EventSource. Generator inside
  `api_stream` ([web_server.py:1612](web_server.py:1612)) does
...
`api_generate_dialogue` at [web_server.py:1398](web_server.py:1398) — uses the
...
- `build_remediation_advisory` (new, `llm/negative_prompts.py:52`) — called from
  `generate_keyframe_take` (`cinema/shots/controller.py:571`) and `diagnose_clip`
