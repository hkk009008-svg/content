...
> **Verification history.** The original map was cross-checked against source
> by parallel investigators. The durable production-control additions and all
> executable anchors were re-verified on 2026-08-05. If you find a divergence
> between this file and the code, fix this file in the same change that exposes
> the drift.
>
...
- §2A [Durable production control plane](#2a-durable-production-control-plane)
...
Operator drives the pipeline through a web UI. `POST /generate` accepts one
idempotent active job per project into a filesystem-backed SQLite queue
([`api_generate`](web_server.py:3310),
[`PipelineJobStore.enqueue`](pipeline_jobs.py:346)). A fixed worker pool then
claims jobs up to the configured global concurrency and runs
`CinemaPipeline.generate()` through the existing checkpoint path
([`PipelineJobDispatcher`](pipeline_jobs.py:769),
[`_execute_pipeline_job_traced`](web_server.py:3213)). That orchestrator
sequences story prep, three Phase classes (keyframe → performance → motion),
assembly, and five operator review gates.

```
[OPERATOR] ──HTTP──▶ web_server.py (Flask + durable job queue)
                         │  fixed worker pool (1 by default; 1..8)
                         ▼
...
|---|---|---|
| HTTP | [web_server.py](web_server.py) | Flask + SSE + CORS. Idempotently enqueues full-project runs and hosts the fixed dispatcher. Caches `PipelineCore` per project (lock-guarded). |
| Durable run queue | [pipeline_jobs.py](pipeline_jobs.py) | SQLite/WAL job truth, one active job per project, FIFO claims, leases, process-liveness fences, fixed global concurrency, checkpoint-resume recovery. |
| Paid-call authority | [paid_provider.py](paid_provider.py), [cost_tracker.py](cost_tracker.py) | Adapter-opt-in paid-attempt reservation, durable provider IDs where available, fail-closed no-replay fences where they are not, budget reservation and terminal reconciliation. |
| Observability | [web_observability.py](web_observability.py), [cinema/trace_store.py](cinema/trace_store.py), [domain/provider_health.py](domain/provider_health.py) | Project-scoped provider analytics/health plus bounded, redacted, searchable local traces. |
| Artifact history & delivery | [cinema/artifact_versions.py](cinema/artifact_versions.py), [cinema/artifact_indexing.py](cinema/artifact_indexing.py), [web_artifacts.py](web_artifacts.py) | Immutable content-addressed objects, hash-chained provenance records, historical-version selection, deterministic allowlisted client ZIPs. |
...
| Shot work | [cinema/shots/controller.py](cinema/shots/controller.py) | `generate_keyframe_take`, `generate_performance_take`, `generate_motion_take`, `regenerate_shot`, `restart_shot`, `diagnose_clip`, `apply_correction`, `generate_scene_preview`. |
...
| LLM | [llm/](llm/) — `ensemble.py`, `chief_director.py`, `style_director.py`, `prompt_optimizer.py`, `negative_prompts.py` | Parallel quorum (not fallback), then a judge picks. Anthropic + OpenAI + **Gemini opt-in**. Real project trackers reserve a deterministic no-replay paid attempt before each planning SDK call, then reconcile token-derived list-price cost and request evidence. These synchronous APIs expose no durable provider job ID, so a crash/ambiguous response is fenced rather than polled or resubmitted. |
...
## 2A. Durable production control plane

The production UI exposes all seven controls; none is only a hidden library:
queue state and its exceptional action are on the Run page
([`RunPage`](web/src/components/pages/RunPage.tsx:175)); provider analytics and
trace search are in its telemetry rail
([`RunPage`](web/src/components/pages/RunPage.tsx:349)); artifact history,
version selection, and packaging are below the final Preview
([`PreviewPanel`](web/src/components/PreviewPanel.tsx:36)).

| Capability | Mechanism and source anchor | Truth boundary |
|---|---|---|
| Durable job persistence | `PipelineJobStore` uses short SQLite/WAL transactions, a partial unique index for one active job per project, stable job IDs, leases, heartbeats, and process-liveness `flock` fences ([pipeline_jobs.py:123](pipeline_jobs.py:123), [pipeline_jobs.py:346](pipeline_jobs.py:346)). Expired jobs whose prior owner is provably stopped are requeued with `resume_required=1` and re-enter the ordinary checkpoint path. | Queue recovery prevents a second *full-project job* from being accepted and resumes local stage progress. It is not, by itself, an exactly-once guarantee for every external provider call; paid-work duplication is controlled at the adapter boundary described below. |
| Provider usage analytics | `CostTracker.get_provider_usage_analytics` aggregates durable paid-media and planning attempts plus Tavily/Firecrawl observations by engine/provider: success rate, latency, failures, reservations, media reconciliation, and known token-priced cost ([cost_tracker.py](cost_tracker.py)). Claude Vision identity uses the paid-attempt fence and reconciles SDK token usage. `GET /api/projects/<pid>/provider-analytics` exposes project or routing-history scope ([web_observability.py:98](web_observability.py:98)). | `cost_basis` is `reconciled_estimate`, not invoice truth. Planning cost is derived from returned tokens and repository list-price tables; media cost is a terminally reconciled estimate; provider invoices remain authoritative. Research wrappers expose outcome/latency but do not invent an unknown vendor charge. |
| Safe multi-project queue | `PipelineJobDispatcher` owns a fixed worker pool and uses the store's atomic oldest-row claim only while the global running count is below `PIPELINE_QUEUE_CONCURRENCY` ([pipeline_jobs.py:769](pipeline_jobs.py:769)). | Concurrency is global to the shared queue database, 1 by default and validated to 1..8. Direct per-project stage/admin fences still serialize conflicting writes. |
| Artifact versioning | Accepted takes, generated character references/embeddings, auxiliaries/finals, and paid Gemini/motion/lip-sync candidates rejected by local gates flow through the provenance adapters ([cinema/artifact_indexing.py](cinema/artifact_indexing.py)). `ArtifactVersionStore` retains read-only content-addressed bytes and append-only hash-chained records ([cinema/artifact_versions.py:350](cinema/artifact_versions.py:350), [cinema/artifact_versions.py:692](cinema/artifact_versions.py:692)). | Provenance captures output/source/dependency hashes and available recipe/provider/model/seed data. Records deliberately say `bit_exact=false`: nondeterministic providers and codec/platform behavior may prevent byte-identical regeneration. If rejected-byte retention fails, fallback is blocked and recovery bytes remain in place. Acquired web reference inputs and project-revision LLM JSON remain project data, not generated binary artifact versions. |
| Automatic provider health | `assess_provider_health` deterministically scores durable paid attempts and request observations, remaining `unknown` below the sample floor unless a hard failure/quarantine signal applies ([domain/provider_health.py:36](domain/provider_health.py:36)). The `AUTO` base-video path removes only `unhealthy` engine candidates; `unknown` and `degraded` remain eligible ([phase_c_ffmpeg.py](phase_c_ffmpeg.py)). | Automatic avoidance currently governs the base-video `AUTO` route. It does not silently reroute a pinned engine, planning LLM, image, lipsync, or performance request; planning health is visible evidence only. |
| One-click client packaging | `ClientPackageBuilder` accepts current or explicit historical client-deliverable IDs, verifies retained bytes, writes a deterministic allowlisted ZIP with `MANIFEST.json` and `SHA256SUMS.txt`, and publishes it under its content hash ([cinema/artifact_versions.py:850](cinema/artifact_versions.py:850), [cinema/artifact_versions.py:937](cinema/artifact_versions.py:937)). The download endpoint requires that hash and streams the same verified open file descriptor ([web_artifacts.py:185](web_artifacts.py:185)). | Only allowlisted client media below `exports/` can enter a package; internal artifacts, credentials, hidden files, and runtime paths are refused. An older content-addressed URL continues to name its original bytes after a new package is built. |
| Searchable traces | `setup_logging` keeps JSON stdout and adds `SQLiteTraceHandler`; the handler redacts credential-shaped text/fields, applies bounded retention, and indexes correlation/project/scene/shot/provider fields ([cinema/logging_config.py:97](cinema/logging_config.py:97), [cinema/trace_store.py:200](cinema/trace_store.py:200)). Project-scoped search is available at `GET /api/projects/<pid>/traces` ([web_observability.py:127](web_observability.py:127)). | This is the application's central local search index, not an external distributed log service. Logging failures never alter production control flow, and stdout remains the deployment log authority. |

### Paid-media recovery boundary

The no-duplicate-paid-work contract is explicit capability ownership, not broad
duck typing. `has_paid_attempt_authority` admits only a tracker implementing the
versioned paid-attempt ledger ([paid_provider.py:41](paid_provider.py:41)).
Adapters with durable IDs resume the same FAL request or ComfyUI prompt after a
restart ([paid_provider.py:283](paid_provider.py:283),
[paid_provider.py:515](paid_provider.py:515)). This includes the FAL
`FAL_SVD` branch and native Kling after its task ID has been acknowledged.
Native Sora's synchronous `create_and_poll` does not expose a safely persisted
job ID at this boundary: once submission starts, an uncertain result becomes
`accepted_unknown` and blocks fallback/replay.

Character creation uses the same boundary. The UI holds one 32-hex
`creation_request_id` across retries; before paid dispatch, the server persists
the project-level pending reservation plus a private, fingerprinted recovery
sidecar containing the staged inputs. The same token and unchanged inputs can
resume an acknowledged FAL request or repair pending `character_reference`
artifact records without resubmitting provider work. `GET .../characters/pending-creation`
exposes the safe status. `DELETE` on that route requires the exact token and
the explicit `reconciled_no_resumable_paid_work` confirmation; it is an
operator reconciliation action, not a retry shortcut. `paid_work_pending`
permits the same-token retry; `paid_work_reconciliation_required` blocks it.
An API that exposes neither a durable job ID nor a documented idempotency key
uses the no-replay fence: after the call boundary is entered, an ambiguous
outcome becomes `accepted_unknown` and automatic fallback stops
([paid_provider.py:148](paid_provider.py:148)). Operator/provider reconciliation
is then required before authorizing new paid work.

Claude Vision identity uses that same no-replay fence. Successful SDK token
usage is reconciled into project/provider analytics; a transport failure after
submission becomes `accepted_unknown`, fails identity closed, and cannot
silently retry or start replacement paid work.

The full-project queue and the paid-attempt ledger solve different crash
windows: the queue resumes orchestration; the adapter ledger resumes or blocks
the exact provider operation. Neither converts an inherently nondeterministic
provider into a bit-exact renderer.

### Planning LLM paid-attempt boundary

`PipelineCore` shares one project-scoped `CostTracker` with `LLMEnsemble` and
`ChiefDirector` ([cinema/core.py:99](cinema/core.py:99)); the directorial,
style, and scene-decomposition paths receive that tracker too. A real tracker
uses `run_fenced_llm_call` to reserve one deterministic attempt and a
conservative token-cost upper bound before the SDK boundary. Success reconciles
that row once from returned usage (including Anthropic cache-write/read rates);
an exception or unusable response after submission becomes `accepted_unknown`.
The same logical retry then raises `PaidCallDeferred` without reaching the SDK,
and tool loops cannot swallow the first ambiguity and issue a replacement
final call.

This is deterministic no-replay protection, **not provider-ID resume**. The
synchronous planning APIs do not expose a durable job ID that the application
can poll after a crash, so a succeeded or ambiguous attempt is never
automatically resubmitted. Planning outcome evidence remains visible in health
analytics but do not alter automatic routing; provider-health avoidance is
still base-video `AUTO` only. Tavily/Firecrawl vendor charge remains unknown
unless the provider supplies authoritative usage; analytics do not convert a
request count into a fabricated dollar amount.

### Truthful live canaries

The protected manual canary keeps two RunPod contracts separate. The
`runpod-pulid-production` target submits the shipping `pulid.json` to the
pinned production image; `runpod-liveportrait-performance` requires its own
`PERFORMANCE_COMFYUI_*` endpoint containing LivePortrait/VHS nodes
([scripts/live_contract_canary.py:59](scripts/live_contract_canary.py:59),
[scripts/live_contract_canary.py:71](scripts/live_contract_canary.py:71)). A
PuLID-ready image is therefore never presented as evidence for the performance
contract. See [docs/LIVE_CONTRACT_CANARY.md](docs/LIVE_CONTRACT_CANARY.md).

---

## 3. Entry point — `web_server.py`

`/assemble` and `/proceed-assembly` intentionally share one handler. Registered
blueprints add the observability and artifact/delivery routes
([web_server.py](web_server.py), [web_observability.py](web_observability.py),
[web_artifacts.py](web_artifacts.py)). Generated product-surface inventory is
the authoritative machine-readable route list; prose does not duplicate a
count that changes whenever a route is added or removed.

### 3.1 Route inventory (grouped)

The generated inventory is authoritative for exhaustive counts. The durable
control surfaces are grouped here so new callers use the correct contract:

| Group | Representative routes |
|---|---|
| Static frontend | `GET /`, `GET /<path>` |
| Global config / cost | `GET /api/config`, `POST /api/cost-estimate`. There is deliberately no global `POST /api/cleanup-all`; cleanup and deletion are project-scoped. |
| Project CRUD and content | `POST/GET /api/projects`, `GET/PUT/PATCH/DELETE /api/projects/<pid>`, characters, objects, locations, scenes, dialogue, style rules |
| Durable lifecycle + SSE | `POST .../generate` (202 + stable queue job), `GET .../pipeline-state`, `GET .../stream`, `POST .../cancel`, `POST .../pause`, `POST .../resume`, `GET .../checkpoint` |
| Exceptional queue recovery | `POST .../queue/abandon` — exact 32-hex job ID plus `acknowledge_paid_work_risk: true`; legal only for an expired running lease whose owner fence cannot be verified ([web_server.py:3584](web_server.py:3584)) |
| Paid-attempt operations | `GET .../cost-live`, `POST .../paid-attempts/<attempt_id>/cancel` |
| Provider analytics and traces | `GET .../provider-analytics`, `GET .../traces` ([web_observability.py:98](web_observability.py:98), [web_observability.py:127](web_observability.py:127)) |
| Artifacts and client delivery | `GET .../artifacts`, `POST .../deliverables/package`, hash-qualified `GET .../deliverables/package/download` ([web_artifacts.py:88](web_artifacts.py:88), [web_artifacts.py:133](web_artifacts.py:133), [web_artifacts.py:185](web_artifacts.py:185)) |
| Gate approvals + shot operations | plan/keyframe/performance/final approval, generation, recovery resolution, restart, regenerate, iterate, diagnose, correct |
| Assembly + preview + files | `/assemble`, `/assemble/re-assemble`, `/assemble/screen`, `/screening/approve`, `/preview/<sid>`, `/export`, `/file` |
| Capability reporting | `GET .../capability-scorecard` (read-only aggregation; no pipeline spin) |

### 3.2 Threading & lifecycle storage

| Symbol | Lock? | Lives at |
|---|---|---|
| `_progress_queues: dict[pid, _ProjectEventBus]` (broadcast bus w/ replay, §3.3 — not a raw `Queue` since Slice 11a) | [`_pipelines_lock`](web_server.py:392) (writes; reads are lock-free GIL-atomic `dict.get`) | [`_progress_queues`](web_server.py:386) |
| `_running_pipelines: dict[pid, CinemaPipeline]` | [`_pipelines_lock`](web_server.py:393) (writes; reads are lock-free GIL-atomic `dict.get`) | [`_running_pipelines`](web_server.py:387) |
| `_project_operation_locks: dict[path, FileLock]` | Registry guarded by `_project_operation_locks_lock`; each safe project gets a sibling `.<pid>.operation.lock` held across decorated mutation/direct-stage/generation admission routes | [`_project_lock_guard`](web_server.py) |
| `_running_cores: dict[pid, PipelineCore]` | [`_cores_lock`](web_server.py:467) | [`_running_cores`](web_server.py:468) |

Pipeline workers belong to the fixed `PipelineJobDispatcher` pool; `POST
/generate` only commits/returns the durable queue row and wakes that pool.
**Cancellation is cooperative** — the durable row records intent and the local
execution context calls `pipeline.cancel()` when it owns the run. The HTTP
handler returns immediately and provider/gate work may take seconds to wind
down. Per-project operation locks and active queue/local reservations prevent a
conflicting delete, settings mutation, or direct paid stage from being admitted
while that work owns the project; the underlying JSON mutation still uses the
separate project-manager RMW lock.

### 3.3 SSE wiring — per-subscriber broadcast bus with replay (Slice 11a)

`_progress_queues: dict[str, _ProjectEventBus]` ([web_server.py:392](web_server.py:392))
...
- `_ProjectEventBus` ([web_server.py:212](web_server.py:212)) is a per-project
...
- The bus and its replay buffer are process-local transport, not durable job
  truth. After restart, `/stream` checks the SQLite queue; if the project still
  has an active queued/running row it creates a fresh bus, starts/wakes the
  dispatcher, and attaches the subscriber. Events emitted before the restart
  cannot be replayed from that new bus, so clients hydrate current queue/stage
  truth from `pipeline-state` and then follow new SSE events. An SSE disconnect
  or absent in-memory bus never cancels the durable job.
...
- **No auth.** No application-auth decorator guards the generated route set.
...
`_get_stage_pipeline(pid)` ([web_server.py:730](web_server.py:730)) returns
...
`_pipeline_action_authority(pid)` ([web_server.py:856](web_server.py:856))
...
`api_patch_project_settings` ([web_server.py:1898](web_server.py:1898)) both
...
|---|---|---|
| 1 | `_refresh_project_snapshot()`; early-return if no scenes | [cinema_pipeline.py:462-466](cinema_pipeline.py:462) |
...
Confirmed settings PUT/PATCH operations and project deletion atomically
exclude generation, evict the idle cached core, and close its `CostTracker`.
Direct out-of-band `project.json` edits are not observed until a supported
mutation or process restart.
...
An unavailable, malformed, or missing verdict is persisted as
`REVIEW_REQUIRED`, so it vetoes PLAN_REVIEW instead of clearing it.
...
| `generate_motion_take` | `workflow_selector.classify_shot_type` + historical `WORKFLOW_TEMPLATES` order → `domain.video_engine_policy.filter_dispatch_candidates` (before budget/TTS/take I/O) → `phase_c_ffmpeg.generate_ai_video` (independent entry fence) → `continuity.validate_shot` → `performance.motion_gate.score_motion_fidelity` | `{success, take, video, identity_score}` or structured `target_api_policy` refusal |
...
8. ChiefDirector validates each scene's shots → APPROVED/MODIFIED/REJECTED,
   or REVIEW_REQUIRED when validation is unavailable or malformed.
...
**Filelock:** 10s default timeout via stock `filelock.FileLock` (bounded
`>=3.24.3,<4` — that line natively provides `O_NOFOLLOW`, the unlinked-inode
`st_nlink` discard, and delete-on-release cleanup, so no local lock subclass
exists). The lock file is a **sibling** of the project directory
(`projects/<pid>.lock`, mode `0o600`), so `delete_project`'s rmtree can never
unlink an actively held lock, and `list_projects` skips it because `.` fails
`is_safe_project_id`. Creation-capable paths (`save_project`, project
creation, and explicit `project_lock`) create the project directory first.
Existing-target `load_project` and `mutate_project` instead perform a
read-only existence/stored-ID preflight, acquire the sibling lock, then
re-read and recheck under lock. A missing or concurrently deleted project
therefore stays absent (the transient sibling lock file is removed on
release). The stored project ID must equal the route/path ID before lock and
under lock, and `mutate_project` reasserts it after the callback before
save/snapshot sync, preventing a route-scoped callback or corrupt file from
redirecting `_save_project_unlocked`.
...
|---|---|---|
| `decompose_scene` ([domain/scene_decomposer.py:838](domain/scene_decomposer.py:838)) | **GPT-4o only**, via `web_research.run_with_tools` (Tavily + Firecrawl, `max_tool_rounds=2`) | fallback to `_fallback_decompose` |
| `competitive_decompose_scene` ([domain/scene_decomposer.py:998](domain/scene_decomposer.py:998)) | `LLMEnsemble.competitive_generate(task_type="decompose", ...)` — Anthropic + OpenAI in parallel + judge | fallback to single-model |
...
`api_generate_dialogue` at [web_server.py:3843](web_server.py:3843) — uses the
...
→ 768×1344), and final delivery node 502 is transposed from 2688×1536 to
1536×2688; FAL Kontext/Pro send `fal_aspect_ratio(…)`, schnell sends
...
`RunPodComfyUI` class ([comfyui_client.py:199](comfyui_client.py:199)), imported
by `phase_c_assembly.py`:

| Method | Endpoint | Notes |
|---|---|---|
| `preflight(workflow)` | `GET /object_info`, `/models`, `/queue` | Rejects unavailable classes, inputs, and model choices before submission |
| `upload_image(image_path)` | `POST /upload/image` | Pooled client with bounded connect/read timeouts |
| `queue_prompt(workflow)` | `POST /prompt` | One attempt; parses `error`/`node_errors` immediately |
| `wait_for_completion(prompt_id)` | `GET /history/{id}`, `WS /ws` | Terminal events fail immediately; bounded history fallback |
| `cancel_prompt(prompt_id)` | `POST /api/jobs/{id}/cancel` | Atomic ID-scoped cancel; an older-pod `/queue` request remains `UNKNOWN` and blocks provider fallback |
| `interrupt()` | `POST /interrupt` | Explicit global control; not used automatically because it can race into the next job |
| `download_image(...)` | `GET /view` | Size/MIME/magic/dimension validation plus atomic publication |

The default job deadline is 600 seconds. WebSocket monitoring falls back to
bounded 2-second history polling; the image cascade continues only after a
terminal job error or confirmed cancellation. Lost/malformed/5xx prompt
acknowledgements and every unconfirmed cancellation are `UNKNOWN` and fail
closed, preventing duplicate FAL work. Idempotent reads retry transport failures, 429,
and selected 5xx responses through shared connection pools. Mutating POSTs,
especially `/prompt`, are never blindly retried. `COMFYUI_API_KEY` optionally
adds bearer authentication for a reverse proxy.

`ShotController.generate_keyframe_take` also writes a durable
`deferred_keyframe_job` reservation immediately before provider dispatch. A
lost ComfyUI acknowledgement, unknown monitor/cancel outcome, worker crash, or
completed response with no publishable local image leaves that reservation in
place, stops `KeyframeRenderPhase`, and blocks every keyframe-creation control
in Review. A private per-attempt token makes every replace, clear, and take
registration conditional, so a late response from an expired/reconciled
request cannot erase or publish over a newer claim. A crash-time
`submission_claimed` marker also carries a 660-second active window (the
600-second ComfyUI deadline plus cancellation/history margin); manual clear is
rejected while that window is live. The UI exposes only the sanitized provider status/job ID and the
explicit `POST .../keyframes/recovery/resolve` action. That action requires
`{"confirmed": true}` and appends an operator-resolution diagnostic; it is used
only after the ComfyUI queue/history and any recoverable output or billing have
been reconciled. A successful take clears the reservation atomically with take
registration. Idle `/pipeline-state` derives `KEYFRAME_REVIEW` from the durable
marker, so recovery controls remain reachable after reload or server restart.
...
non-empty, `_fal_flux_fallback` ([phase_c_assembly.py:1040](phase_c_assembly.py:1040)) takes the multi-char
branch: `_allocate_ref_slots` ([phase_c_assembly.py:975](phase_c_assembly.py:975)) partitions the Kontext
...
([phase_c_assembly.py:523](phase_c_assembly.py:523)) emits per-character `@ImageN PRESERVE` blocks with a
...
## 9. Video routing — 5 order templates × typed dispatch policy

### 9.1 `WORKFLOW_TEMPLATES` ([workflow_selector.py:29-135](workflow_selector.py:29))
...
These values are **automatic order seeds, not executable truth**.
...
`filter_dispatch_candidates` ([domain/video_engine_policy.py](domain/video_engine_policy.py))
now gates it on runtime availability only. `ShotController.generate_motion_take` prepends a cached
optimizer suggestion only when its lifecycle is active and it is dispatch-safe,
then filters the whole ordered seed once using
...
fallbacks for the mandatory downstream TTS/lipsync pass. Deprecated
`SORA_NATIVE` is absent from automatic/template seeds; an operator may still
pin it explicitly before 2026-09-24, after which policy date-retires it.
`SORA_2` is always retired.

### 9.2 `classify_shot_type` keyword map ([workflow_selector.py:174-218](workflow_selector.py:174))
...
### 9.3 `MOTION_FIDELITY_FLOORS` ([workflow_selector.py:159-165](workflow_selector.py:159))
...
The table below inventories executable payload branches. Retired IDs remain
catalog tombstones for historical projects but no longer retain payload code.

| Engine | File:line | Adapter | Auth |
|---|---|---|---|
| `KLING_NATIVE` | :529 | `kling_native.KlingNativeAPI` (legacy `kling-v1-6`; explicit-only compatibility). The paid ledger persists an acknowledged task ID and an identical retry polls that exact task; a lost submit acknowledgement becomes `accepted_unknown`. | JWT HS256 (KLING_ACCESS_KEY + KLING_SECRET_KEY) |
| `SORA_NATIVE` | :599 | `sora_native.SoraNativeAPI`; explicit image-only compatibility before its 2026-09-24 sunset. `create_and_poll` has no durable application-owned job-ID handoff, so uncertainty after submission is `accepted_unknown`, never fallback. | OPENAI_API_KEY |
| `VEO_NATIVE` | :672 | `veo_native.VeoNativeAPI`; Vertex only with an explicit project and resolvable ADC, otherwise Developer API when a key exists | GOOGLE_CLOUD_PROJECT + ADC, or GOOGLE_API_KEY |
| `GEMINI_OMNI` | :1421 | `gemini_omni_native.GeminiOmniAPI` — inline base64 decode, `files/<id>` polling, terminal classification, and bounded ffprobe-validated atomic publication; runtime-gated | GOOGLE_API_KEY or GEMINI_API_KEY |
| `LTX` | :738 | `ltx_native.LTXVideoAPI`; native uses LTX signed upload + persisted async-v2 job polling + MIME/container-validated atomic MP4 publication; FAL remains the proxy alternative; rejected for `9:16` by current dispatch-wiring policy | LTX_API_KEY OR FAL_KEY |
| `RUNWAY_GEN4` | :872 | inline `runwayml` SDK (`gen4_turbo`); JPEG/PNG/WebP data URI is content-validated before client construction | RUNWAYML_API_SECRET |
| `VEO` | :991 | inline `fal_client.subscribe("fal-ai/veo3.1/reference-to-video")` | FAL_KEY (gated by `_veo_quota_blocked`) |
| `KLING_3_0` | :1080 | inline `fal_client.subscribe("fal-ai/kling-video/v3/pro/image-to-video")`; `elements` identity (frontal + ≤3 refs) | FAL_KEY |
| `FAL_SVD` | :1199 | FAL fast-SVD through `run_durable_fal_job`; the acknowledged queue request ID is persisted and resumed without another submit. A narrow no-authority compatibility seam still uses `subscribe` for offline utilities/tests. | FAL_KEY; **not in any default/template cascade** |
| `SEEDANCE` | :1320 | inline `fal_client.subscribe("bytedance/seedance-2.0/image-to-video")`, or `.../reference-to-video` when multi-angle refs exist (keyframe first, ≤9 images) | FAL_KEY |

### 9.5 Default AUTO order seed

[phase_c_ffmpeg.py:94](phase_c_ffmpeg.py:94):
```python
...
The list is the module-level `DEFAULT_VIDEO_CASCADE` constant. `GEMINI_OMNI`
is deliberately omitted from this blind order because its duration/resolution/
audio are prompt-inferred. Deprecated `SORA_NATIVE` and `KLING_NATIVE` are
absent from automatic dispatch and remain explicit compatibility pins while
their policy permits. Legacy `RUNWAY` and `SORA_2` are retired catalog
tombstones with no executable payload branch.
The typed filter remains authoritative and may remove additional entries for
date, runtime, project, or aspect constraints.
`try_next_api` and the cooldown retry consume only the already-filtered tuple,
never this raw constant. A direct explicit target with no supplied safe-chain
semantics does not read this list. `FAL_SVD` is reachable only via an eligible
explicit `target_api=`. `VEO_NATIVE` leads the seed and `KLING_3_0` is the
current automatic Kling route.

**FAL client timeouts (2026-06-10):** remaining production
`fal_client.subscribe` compatibility/direct-call sites pass `client_timeout`
from [cinema/fal_limits.py](cinema/fal_limits.py) (video 600s / talking-head
generation 1800s / image 180s). Paid motion, keyframe, character, and other
queue-backed paths instead use bounded `submit`/status/result polling through
`run_durable_fal_job`, which is what preserves the request ID across a local
timeout or restart. The timeout guard remains AST-enforced by
[tests/unit/test_fal_subscribe_timeouts.py](tests/unit/test_fal_subscribe_timeouts.py).

### 9.6 VEO quota gate

**TTL-based** (commit `feccf61`):
- Variable: `_VEO_QUOTA_EXHAUSTED_UNTIL: float = 0.0` ([phase_c_ffmpeg.py:36](phase_c_ffmpeg.py:36))
- TTL: `_VEO_QUOTA_TTL_S: int = 1800` (30 min) ([:37](phase_c_ffmpeg.py:37))
...
### 9.8 LTX duration contract, accepted-job recovery, and duration-true billing

`LTXVideoAPI.DURATION_SECONDS = (6, 8, 10)`
([ltx_native.py](ltx_native.py)), default 6 — both the native
`api.ltx.io` endpoint (`model="ltx-2-3-pro"`) and the FAL proxy
...
**Native lifecycle (2026-08-04).** Native mode no longer uploads through FAL
or holds a synchronous generation connection open. It obtains a signed upload
from `POST /v1/upload`, uploads the MIME-detected PNG/JPEG/WebP bytes, submits
the returned `ltx://` URI to `POST /v2/image-to-video`, atomically persists the
returned job ID in a request-fingerprint sidecar, and polls the exact job on a
bounded 5-second cadence. A later identical invocation resumes that job rather
than submitting duplicate paid work. Completed output is accepted only when
the download reports `video/mp4` and `ffprobe` verifies an MP4-family container,
nonempty video stream, codec, dimensions, and duration; publication uses the
shared sibling-temp + `os.replace` path. Native jobs are
never rerouted through FAL after submission has begun. `LTXJobPending` crosses
the adapter boundary for a running job, an ambiguous submit, corrupt recovery
state, post-ack protocol/network failure, or completed output that cannot be
validated/published. The dispatcher records `_cascade_out["deferred_job"]`
(`status`, `reason`, recovery identifiers, attempts, and billed state) and
returns without entering the next provider or the cooldown retry. Only an
explicit provider terminal `failed` result or a wholly pre-submission failure
remains eligible for the normal cascade. A lost submit response is persisted as
`submission_unknown`, which blocks an identical automatic resubmit.
Valid terminal `failed` and `expired` sidecars may be atomically superseded for
an identical retry. The replacement is serialized by a per-fingerprint file
lock, so concurrent retries cannot both submit; pending, processing,
submission-claimed, and unknown states remain fail-closed.

**Provider-neutral accepted-job fence (2026-08-04).** Gemini Omni and native
Veo now obey the same paid-work boundary even though they do not use the LTX
sidecar. An acknowledged job, an ambiguous submit response, a poll transport or
timeout ambiguity after acceptance, and a completed job whose output cannot be
retrieved or atomically published all produce a sanitized
`deferred_motion_job` on the shot and stop the cascade. The descriptor carries
the exact provider job ID and status needed for reconciliation; Veo also carries
the dispatched duration. Explicit terminal provider failures, safety/RAI
rejections, and failures proven to occur before submission remain eligible for
fallback. The controller reserves pending cost and records a later billed result
idempotently by provider job ID, so a resumed process cannot silently count the
same paid job twice. LTX jobs with an exact ID can be resumed through the same
adapter and private sidecar. Gemini/Veo automatic polling is deliberately not
claimed: the Review UI labels those records **Manual Recovery Required** and
directs the operator to reconcile the exact job in the provider console before
clearing the marker or authorizing new paid work.
...
duration/native lifecycle plus provider-neutral accepted-job recovery and
duration-true billing (§9.8) verified: 2026-08-04.
...
| `"native"` | Requests provider-native voice and sets `video_fallbacks=None`. A take is tagged `audio_embedded=True` only when the winning backend verifies that it generated audio (Gemini Omni or Vertex/ADC Veo). Developer-API Veo is treated as silent and receives F1b. Embedded audio remains unmeasured sync evidence, so final auto-approval requires review rather than a synthetic score. |
...
2. ~~**N=8 best-of grading** — `face_validator_gate._arcface_score` →
   ...
   fail-closed policy). The former standalone manual pod/spend probes were
   removed on 2026-08-05; there is still no `web_server.py` →
   `cinema_pipeline.py` production dispatch.
...
`_assemble_final` ([cinema_pipeline.py:1675](cinema_pipeline.py:1675)) muxes the
...
### 13.1 `LLMEnsemble` ([llm/ensemble.py:170](llm/ensemble.py:170))
...
`_generate_gemini` ([llm/ensemble.py:689](llm/ensemble.py:689)) is fully
...
Returns `{decision, violations, shots}` where `decision ∈ {APPROVED, REJECTED,
MODIFIED, BLOCKED, REVIEW_REQUIRED}`.
...
No client, API failure, a non-object response, parse failure, or a missing/
unsupported decision → pass-through shots with `REVIEW_REQUIRED`; these
outcomes cannot clear PLAN_REVIEW.
...
Decision: `RETRY | ACCEPT_LENIENT | FAIL | REVIEW_REQUIRED`. Missing identity
evidence, no client, or an unavailable/unparsable response requires review.
Negative-prompt phrases (from
...
The `suggested_video_api` enum is rendered from the typed catalog's currently
eligible authoring targets plus `AUTO`; deprecated fallback-only engines such
as `SORA_NATIVE` are not offered for new optimizer choices. Sanitized via
`_coerce_to_valid_keys` — unrecognized or ineligible values collapse to
`"AUTO"`. Non-object JSON roots and coerce
...
macro_detail:          LTX (or another currently eligible ranked engine); lipsync=null
style_locked_sequence: RUNWAY_GEN4;    lipsync per dialogue need
```

### 13.7 `negative_prompts.py`

72 lines. `NEGATIVE_PROMPT_BY_FAILURE_REASON` dict keyed off
...
- `build_remediation_advisory` (new, `llm/negative_prompts.py:55`) — called from
  `generate_keyframe_take` (defined at `cinema/shots/controller.py:1308`) and `diagnose_clip`
...
singleton via `get_settings()` ([config/settings.py:225](config/settings.py:225)).
...
**Bus LIVE; seat-runtime: T1/T2/T3 local CLI mechanism operable; protected-main deployment remains fail-closed pending deployment controls.**

The current local mechanism makes T1/T2/T3 brief→merge paths end-to-end operable, human-driven, against the protected TEST ref only:

- `scripts/overseer_emit.py` — overseer signing CLI; subcommands include `brief`, `assignment`, `cycle_go`, `release_order`, `approver_roster`, `re_verify_challenge`, `brief_superseded`, and overseer `attestation_revoked`. Constructs an `Event(seq=0)` and delegates signing to `RefEventStore.append` (DD-5). Defaults `--remote origin`; catches `AppendContentionExceeded` / `ValueError` → clean exit-1 (DD-4).
- `scripts/seat_emit.py` — per-seat signed emission (`candidate`, `attestation`, `release_requested`, `candidate_aborted`, T2 `co_sign`, T3 `re_verify`, and seat self-revocation via `attestation_revoked`); the seat signs with its OWN key (closes the retired shim's signer-derived key-injection hole). Read side: `scripts/consume_bus.py` (reads bus events past a per-seat cursor). The `bootstrap_emit.py` shim was retired in sub-project 2 (ADR-061).
- `scripts/chief_emit.py` — chief/human signing CLI for rostered `human_approval` and chief self-revocation; it loads the chief key, not the overseer key, and fails rc2/zero-events when the approver is unrostered.
- `scripts/threeway_mechanism_ledger.py` + `docs/protocol/threeway/MECHANISM-LEDGER.md` — executable ledger covering every `LOAD_BEARING_KINDS` member. Verified via `env -u GIT_INDEX_FILE .venv/bin/python scripts/threeway_mechanism_ledger.py --check` -> exit 0.
- `scripts/run_merge_gate.sh` — standing daemon wrapper; passes `--main-ref refs/threeway/test-main` and `--remote origin` to `run_merge_gate.py`; relies on ADR-055 sys.path self-bootstrap (no import-path env export, DD-2). Safety boundary: local `refs/heads/main` use now fails closed unless `--allow-protected-main` is present, and even then returns rc1 until deployment controls are verifiable. Verified via `env -u GIT_INDEX_FILE .venv/bin/python -m pytest tests/unit/test_threeway_run_merge_gate_protected_main.py -q` -> 2 passed in 0.03s.
...
The T1 E2E walking-skeleton (`tests/unit/test_threeway_e2e_walking_skeleton.py`) and T2/T3 walking-skeleton (`tests/unit/test_threeway_t2_t3_emitters_e2e.py`) drive the CLIs + daemon (`--run-once`) against a temp repo and assert the test ref advances only when required signed facts exist. Verified via `env -u GIT_INDEX_FILE .venv/bin/python -m pytest tests/unit/test_threeway_t2_t3_emitters_e2e.py -q` -> 6 passed in 41.34s. See `DECISIONS.md` ADR-056 and ADR-064. The `overseer_plan` automation layer (ADR-057) sits above `overseer_emit`; see `DECISIONS.md` ADR-057.
...
composes **7 sections**
([SettingsInspector.tsx](web/src/components/setup/SettingsInspector.tsx)):
Project, Video, Image, Identity, Voice, Auto-Approve, Budget. Project is defined locally
([SettingsInspector.tsx:65](web/src/components/setup/SettingsInspector.tsx:65));
the other six live in `web/src/components/setup/inspector/`, alongside the
shared row primitives (`controls.tsx`). The obsolete reorderable lip-sync
priority control was removed; Voice now exposes only supported engine and
language behavior, while Auto-Approve owns the measured final lip-sync floor
...
body-parity (`check_skill_twin_parity`, ADR-077 — the `.agents/skills/`
...
| **ChiefDirector** | LLM validator (Anthropic by default, OpenAI fallback). Pre-gen: APPROVED / REJECTED / MODIFIED, with REVIEW_REQUIRED for unavailable or malformed evidence. Post-gen: RETRY / ACCEPT_LENIENT / FAIL / REVIEW_REQUIRED with mutation focus. §13.4 |
