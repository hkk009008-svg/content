...
|---|---|---|
| `_progress_queues: dict[pid, Queue]` | `_pipelines_lock` (writes; reads are lock-free GIL-atomic `dict.get`) | [web_server.py:93](web_server.py:93) |
| `_running_pipelines: dict[pid, CinemaPipeline]` | `_pipelines_lock` (writes; reads are lock-free GIL-atomic `dict.get`) | [web_server.py:93](web_server.py:93) |
| `_running_cores: dict[pid, PipelineCore]` | `_cores_lock` | [web_server.py:123](web_server.py:123) |
...
- `GET /api/projects/<pid>/stream` opens an EventSource. Generator inside
  `api_stream` ([web_server.py:2499](web_server.py:2499)) does
...
`api_generate_dialogue` at [web_server.py:2277](web_server.py:2277) — uses the
...
non-empty, `_fal_flux_fallback` ([phase_c_assembly.py:616](phase_c_assembly.py:616)) takes the multi-char
branch: `_allocate_ref_slots` ([phase_c_assembly.py:551](phase_c_assembly.py:551)) partitions the Kontext
...
### 9.3 `MOTION_FIDELITY_FLOORS` ([workflow_selector.py:173-180](workflow_selector.py:173))
...
2026-07-11 so tests pin the real list). Known-broken `GEMINI_OMNI` and retired
`SORA_2` are absent; the typed filter remains authoritative and may remove
additional entries for date, runtime, project, or aspect constraints.
...
explicit `target_api=`. `VEO_NATIVE` leads the seed; `KLING_3_0` (fal v3 Pro)
outranks the legacy `KLING_NATIVE`.
